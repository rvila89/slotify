...
> Capability de **presentación del pipeline de reservas**: la pantalla `/reservas`
> del frontend con dos tabs — **Kanban** ("Flujo de Reserva") y **Listado** — que
> consume el endpoint de solo lectura `GET /reservas` (`operationId: listarReservas`,
> capability backend `pipeline`, US-049) vía el SDK generado. Es una vista de
> **solo lectura**: NO muta estado, NO ejecuta transiciones (el clic navega a la
> ficha) y NO cambia el contrato OpenAPI. Reemplaza el `SectionPlaceholder` que hoy
> ocupa la ruta `/reservas`. (Fuente: US-050, UC-37, UC-38; schema `Reserva` en
> `docs/api-spec.yml`; estados activos vs terminales en `CLAUDE.md §Máquina de
> estados`.)
