...
- [x] 2.1 Evolucionar el contrato: definir el endpoint de resultado de visita "interesado"
...
- [x] 3.1 Test de la **guarda de origen** en la máquina de estados: `2v → 2b` permitida;
...
- [x] 3.2 Test del use-case desde `2.v`: `→ 2b` + `visita_realizada=true` +
...
- [x] 3.3 Test del **TTL fresco**: con `ttl_expiracion` previo = día post-visita y
...
- [x] 3.4 Test del **registro antes de la fecha de visita** (FA): `visita_programada_fecha > hoy`
      NO bloquea el registro; la transición procede y el TTL se calcula desde `now` (en rojo)
- [x] 3.5 Test de **atomicidad**: fallo parcial → rollback completo (RESERVA en `2v`,
...
- [x] 3.6 **Tests de concurrencia REALES (skill `concurrency-locking`)**: registro de
...
- [x] 3.7 Test del **disparo de E7** y su registro en `COMUNICACION` (`codigo_email='E7'`,
...
- [x] 3.8 Confirmar que toda la batería está **en rojo** antes de implementar (verificado con
...
- [x] 4.1 Revisar tests de US-004/US-005/US-006/US-007/US-008/US-040/US-045 afectados por el
...
- [x] 5.1 Máquina de estados: añadir origen válido `{2v} → 2b` (resultado "interesado")
      + guarda declarativa (tabla, no `if` dispersos): todo origen distinto de `2v` inválido,
      terminales inmutables (D-1)
- [x] 5.2 Reutilizar `resolverPlanBloqueo({fase:'2.b'})`: TTL = `now + ttl_consulta_dias`
...
- [x] 5.3 Use-case `2v → 2b` "cliente interesado": validar guarda de origen; UPDATE de la
      ...
      (D-3)
- [x] 5.4 Disparo de **E7 post-commit** vía el motor de email de US-045 + registro en
      `COMUNICACION` (`enviado`/`fallido`) (D-4)
- [x] 5.5 Read-model `GET /reservas/{id}` expone `subEstado`, `visitaRealizada` y
      `ttlExpiracion` actualizados (reuso de `obtener-reserva.query.ts`); mapper `2v/2b`
      cableado (ya desde US-008)
- [x] 5.6 Endpoint de resultado de visita (controller + DTO `{resultado}`) con mapeo de
      respuestas 200/422/404 (D-5); registrar en `reservas.module.ts`
- [x] 5.7 Frontend "ficha de reserva": acción "Registrar resultado de visita" → opción
...
- [x] 7.4 POST sobre RESERVA en `2a`/`2b`/`2c`/`2d` (no en `2v`) → 422 (guarda de origen);
      RESERVA intacta
- [x] 7.5 POST sobre RESERVA en terminal (`2x`/`2y`/`2z`/`cancelada`/`completada`) → 422; sin
      efectos
...
- [x] 12.1 `openspec archive us-009-resultado-visita-cliente-interesado` (solo tras gate final
