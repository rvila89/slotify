# Spec Delta — Capability `calendario`

> Capability de **vista de lectura agregada** de la disponibilidad de fechas.
> Lectura pura sobre `RESERVA` y `FECHA_BLOQUEADA`, aislada por `tenant_id`.
> Fuente: US-039, UC-29; código de colores SlotifyGeneralSpecs §11.3;
> entidades en `er-diagram.md` (`RESERVA`, `FECHA_BLOQUEADA`, `TENANT`).
> No muta estado; las garantías de concurrencia sobre el bloqueo residen en US-040.
