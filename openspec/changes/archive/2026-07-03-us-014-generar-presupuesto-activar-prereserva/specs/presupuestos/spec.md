# Spec Delta — Capability `presupuestos` (NUEVA)

> US-014 **crea** la capability `presupuestos`: la generación del presupuesto formal de
> una RESERVA como **borrador editable**, el **congelado de la tarifa** (`tarifa_congelada
> = true`) al confirmar, el cálculo del **desglose fiscal** (base imponible + IVA 21% +
> extras + descuentos + total y el reparto 40%/60%/fianza), la generación del **PDF** y el
> caso de **precio manual** cuando el motor devuelve `tarifa_a_consultar` (>50 invitados).
> El agregado raíz de esta capability es **PRESUPUESTO** (no RESERVA). El cálculo de la
> tarifa se **delega** al motor de la capability `calculo-tarifa` (US-016), sin reinventar
> la lógica de tarifario. La **transición de estado** de la RESERVA a `pre_reserva`, el
> bloqueo de fecha y el vaciado de cola se especifican en el delta de la capability
> `consultas`; el disparo de E2 con el PDF adjunto, en el delta de `comunicaciones`. Los
> tres deltas se coordinan en **una única transacción** en el use-case de UC-14.
> Fuente: US-014, UC-14; UC-16/US-016; `er-diagram.md §3.11 PRESUPUESTO`; `CLAUDE.md`.
