...
Capability de **búsqueda, filtrado y consulta en modo lectura del histórico de
reservas terminadas**. Expone el endpoint `GET /historico`
(`operationId: listarHistorico`) que devuelve la lista **paginada** de reservas
en estado **cerrado** (`reserva_completada` por defecto, `reserva_cancelada` como
opt-in) del tenant, con **filtros estructurados** (rango de fecha de evento, tipo
de evento, estado final, rango de importe) y **búsqueda full-text** sobre datos
del cliente, código y notas. El acceso al detalle de cada registro reutiliza
`GET /reservas/{id}` (`ReservaDetalle`) y se presenta en **modo lectura**
(inmutable). Es una operación de **lectura pura**: NO muta estado, NO produce
bloqueos y NO introduce transiciones. La capability `consultas` sigue siendo
dueña del ciclo de vida del agregado `RESERVA`; la capability `pipeline` lee las
reservas **activas**; `historico` lee su **complemento cerrado**. (Fuente:
US-042, UC-32; entidades `RESERVA`/`CLIENTE` en `er-diagram.md`; estados cerrados
en `CLAUDE.md §Máquina de estados`.)
