...
>
> **ESTADO ACTUAL: gate SDD APROBADO (2026-07-10).** El paso 0 (branch) y los artefactos (proposal +
> spec-delta + design) están hechos, y el humano ha aprobado el gate SDD resolviendo las 4 decisiones
> abiertas del `design.md` (ver §1.2: D-2=A, D-3+D-4=3.1+4.2, D-5=visibilidad/filtrabilidad). El
> siguiente paso es el contrato (paso 2); **NO se implementa nada más en este registro** — esta
> edición solo documenta la resolución del gate.
...
- [ ] 2.1 Definir en `docs/api-spec.yml` el **endpoint DEDICADO NUEVO** `POST /cron/barrido-completadas`
...
- [ ] 2.2 Definir el schema `BarridoCompletadasResponse` (`{ candidatas, archivadas, fianzaPendiente,
      fallos }`, todos `required`, enteros ≥ 0), estilo `BarridoEventosResponse`/`BarridoExpiracionResponse`.
- [ ] 2.3 `spectral lint docs/api-spec.yml` en verde (0 errores nuevos frente al baseline; hook
      `validate-openapi`).
- [ ] 2.4 Regenerar el SDK del frontend (`pnpm generate-client`; **nunca** editar el cliente
...
- [ ] 3.1 Añadir `RESERVA.fechaPostEvento` (`DateTime?`, `@map("fecha_post_evento")`) al
      `schema.prisma` y crear la **migración no destructiva** (columna nullable). Regenerar el cliente
      Prisma.
- [ ] 3.2 **Modificar US-034** para poblar `fechaPostEvento` en la transición a `post_evento` (misma
      transacción que fija `estado = post_evento`). **Coordinación cross-US** (alcance aprobado en el
      gate SDD, §1.2).
- [ ] 3.3 Definir el **backfill** del residual (RESERVA ya en `post_evento` antes de la migración):
      derivar de `AUDIT_LOG` la fecha de la transición a `post_evento` (mecanismo aprobado en el gate,
      §1.2). Documentar la estrategia.
- [ ] 3.4 (No aplica: D-2=A aprobado; se descartan las opciones B/AUDIT_LOG-en-caliente y
      C/`fechaActualizacion`.)
- [ ] 3.5 (No aplica: D-3=3.1 + D-4=4.2 aprobados → alerta y anti-duplicación por `AUDIT_LOG`, **sin
...
- [ ] 4.1 Test del **mapa/guarda de origen declarativos** del archivado en `maquina-estados.ts`:
...
- [ ] 4.2 Test de la **guarda pura de fianza** (`fianzaResuelta`): `true` si `fianzaStatus ∈
...
- [ ] 4.3 Test del use-case **happy path — fianza devuelta**: candidata `post_evento` + T+7d +
...
- [ ] 4.4 Test **sin fianza** (`fianza_eur = 0` o `NULL`) + T+7d → archiva sin evaluar `fianza_status`
      (en rojo).
- [ ] 4.5 Test **retención total** (`retenida_parcial` + `fianza_devuelta_eur = 0`) + T+7d → archiva
      (en rojo).
- [ ] 4.6 Test **FA-01 — fianza no resuelta en T+7d** (`fianza_status = cobrada`, `fianza_eur > 0`)
      ...
      (D-3=3.1) (en rojo).
- [ ] 4.7 Test **anti-duplicación de la alerta** (D-4=4.2, por `AUDIT_LOG`): 2.º barrido sobre la misma
      ...
      sí re-emite (en rojo).
- [ ] 4.8 Test del **filtro estricto por estado**: RESERVA en `consulta`/`pre_reserva`/
      `reserva_confirmada`/`evento_en_curso`/`reserva_cancelada`/`reserva_completada` → NO se archiva
      (en rojo).
- [ ] 4.9 Test del **filtro por antigüedad** (D-2=A, `fechaPostEvento`): solo `post_evento` con
      ...
      (en rojo).
- [ ] 4.10 Test **FA-02 — idempotencia**: RESERVA ya en `reserva_completada` (pase previo o US-038) →
      no candidata, cero cambios, cero auditorías; 2.ª ejecución del barrido → no re-archiva (en rojo).
- [ ] 4.11 Test de **múltiples reservas**: 2 resueltas → 2 transiciones (2 auditorías independientes),
      1 fianza pendiente → alerta sin transición, 1 ya `reserva_completada` → omitida; resumen = 2
      archivadas + 1 fianza pendiente (en rojo).
- [ ] 4.12 Test de **atomicidad / fallo aislado**: fallo en una candidata → rollback solo de esa; las
      demás se archivan; el resumen refleja el fallo aislado (en rojo).
- [ ] 4.13 **Tests de concurrencia (skill `concurrency-locking`)**: **RC-1** doble barrido sobre la
      ...
      (en rojo).
- [ ] 4.14 Test del **endpoint/guard**: `X-Cron-Token` ausente/inválido → 401; token válido → 200 con
      resumen `BarridoCompletadasResponse` (en rojo).
- [ ] 4.15 Confirmar que toda la batería está **en rojo** antes de implementar (por ausencia de
...
- [ ] 5.1 Revisar tests de la máquina de estados de RESERVA (US-034 `post_evento`, US-031
...
- [ ] 6.1 Máquina de estados (`reservas/domain/maquina-estados.ts`): añadir la **arista/guarda de
...
- [ ] 6.2 Caso de uso `ArchivarReservasCompletadasService` (aplicación) con `ejecutar()`: listar
...
- [ ] 6.3 Infra: adaptador Prisma para listar candidatas cross-tenant (selección por antigüedad por
...
- [ ] 6.4 Endpoint/cron DEDICADO: `BarridoCompletadasController` (`POST /cron/barrido-completadas`,
...
- [ ] 7.1 Capturar baseline de BD (counts/estado de `reserva`, `audit_log`; sembrar candidatas:
...
- [ ] 7.2 Ejecutar tests dirigidos de los módulos cambiados (incl. concurrencia RC-1/RC-2) **desde la
      sesión principal con Postgres real** (los subagentes QA no tienen BD).
- [ ] 7.3 Ejecutar la suite requerida (`pnpm test`); anotar el flaky conocido de US-004 (`40P01`) si
      aparece, sin atribuirlo a este change.
- [ ] 7.4 Verificar estado posterior de BD (candidatas resueltas → `estado=reserva_completada`;
...
- [ ] 7.5 Crear report `reports/YYYY-MM-DD-step-N+1-unit-test-and-db-verification.md`.
- [ ] 7.6 Marcar completado solo tras tests en verde y report creado.
...
- [ ] 8.7 Crear report `reports/YYYY-MM-DD-step-N+2-curl-endpoint-tests.md`.

## 9. QA: E2E con Playwright MCP (step-N+3 — NO APLICA: sin frontend — EL AGENTE DEBE JUSTIFICARLO)
- [ ] 9.1 US-037 no introduce UI propia (actor Sistema, job cron backend puro; el módulo Histórico
...
- [ ] 10.1 Actualizar docs técnicas: capability `consultas` (transición automática a
...
- [ ] 11.1 Ejecutar `code-reviewer` sobre el diff (guardrails: hexagonal, sin bloqueo distribuido, sin
...
- [ ] 11.2 Dejar informe `reports/YYYY-MM-DD-step-review-code-review.md` con la línea literal
...
- [ ] 13.1 `openspec archive us-037-archivado-automatico-reserva-completada` (solo tras gate final y
      code-review APTO; el hook `require-code-review` lo bloquea sin APTO).
- [ ] 13.2 Actualizar `openspec/specs/` (capability `consultas` con los requisitos de archivado
      automático) y abrir PR (GitHub MCP / `gh`); registrar el nº de PR en el front-matter de la US.
