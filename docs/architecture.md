...
#### Actualización de DT-EMAIL-02 (E3) — RESUELTA en US-023 (6.4b); condiciones movidas a E2 en change `condiciones-idioma-e2-firma-banner`

El email E3 ha sido implementado en US-023 (change `documentos-enviar-factura-senal-e3`, 6.4b). El Gestor pulsa "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`): en una única operación atómica, aprueba la factura de señal (`borrador → enviada`) y envía E3 por `EnviarEmailPort` directo (rollback total si el proveedor falla) con la factura de señal como único adjunto; crea COMUNICACION E3 `enviado`; registra AUDIT_LOG. Idempotente: 409 `E3_YA_ENVIADO` si ya existe COMUNICACION E3 `enviado`.

**Cambio del change `condiciones-idioma-e2-firma-banner`:** las condicions particulars **ya no se adjuntan ni se envían en E3**. Desde este change, `GenerarPresupuestoUseCase.confirmar()` realiza una guarda pre-transacción que llama a `generarCondicionesPort.generar({ tenantId, idioma })`: si devuelve `null` → `CondicionesNoConfiguradasError` (409 `CONDICIONES_NO_CONFIGURADAS`) sin ningún efecto; si tiene éxito → dentro de la transacción se fijan `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false`. El PDF de condiciones se adjunta al email E2 (presupuesto). El guard duro `CONDICIONES_NO_CONFIGURADAS` se aplica ahora en `POST /reservas/{id}/presupuesto`, no en el envío E3. Adicionalmente, `GenerarPdfCondicionesPort.generar()` ahora requiere el parámetro `idioma: 'es' | 'ca'`; la clave de storage incluye el idioma (`condiciones/{tenantId}-{idioma}.pdf`). Ver DT-EMAIL-02 en §2.9.
