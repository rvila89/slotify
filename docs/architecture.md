...
#### Actualización de DT-EMAIL-02 (E3) — RESUELTA en US-023 (6.4b)

El email E3 ha sido implementado en US-023 (change `documentos-enviar-factura-senal-e3`, 6.4b). El Gestor pulsa "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`): en una única operación atómica, aprueba la factura de señal (`borrador → enviada`), adjunta las condicions particulars si están configuradas (degrada a `null` sin error), envía E3 por `EnviarEmailPort` directo (rollback total si el proveedor falla), fija `RESERVA.cond_part_enviadas_fecha`, crea COMUNICACION E3 `enviado` y registra AUDIT_LOG. Idempotente: 409 `E3_YA_ENVIADO` si ya existe COMUNICACION E3 `enviado`. Ver DT-EMAIL-02 en §2.9.
