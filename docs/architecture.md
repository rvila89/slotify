...
graph TB
    ...
    MAIL["Proveedor de email<br/>Plantillas E1-E9"]
...
- **E3:** activa (6.4b/US-023). Render real en `es` y `ca` (bilingüe). Envío por `EnviarEmailPort` directo (sin `DespacharEmailService`) para garantizar rollback atómico. `adjuntosRequeridos`: factura de señal (requerida); PDF condicions particulars (degradable: `.catch(() => null)` — omitido si el tenant no tiene condicions configuradas o el render falla). **Desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** E3 adjunta el PDF de condicions particulars en blanco de forma degradable vía `GenerarPdfCondicionesPort.generar({ tenantId, idioma })`; si se adjuntan, se fijan `RESERVA.cond_part_enviadas_fecha` y `cond_part_firmadas = false` dentro de la transacción; E2 ya no adjunta condicions. Se eliminan el guard duro `CONDICIONES_NO_CONFIGURADAS` (409) y la clase `CondicionesNoConfiguradasError`. La plantilla incluye el párrafo sobre condicions **solo si** `condicionesAdjuntas === true`. Trigger: acción manual del Gestor "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`).
...
- **E4:** activa (ver §2.15 actualización). **E4 = solo liquidación** (change `fix-liquidacion-fianza-independientes`): adjunta únicamente el PDF de la factura de liquidación. El texto bilingüe CA/ES recuerda al cliente abonar la fianza antes o el día del evento. **Desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** la plantilla E4 incluye un **párrafo recordatorio** al cliente de que las condicions particulars firmadas están pendientes de devolver **solo si** `RESERVA.cond_part_firmadas = false`; si `cond_part_firmadas = true`, el párrafo se omite. El flag se propaga como `recordarCondicionesPendientes = !condPartFirmadas` al render. Modo síncrono (rollback si falla el proveedor).
...
El email E3 se dispara mediante la acción manual del Gestor **"Enviar factura 40%"** implementada en US-023 (`POST /reservas/{id}/facturas/senal/enviar`, change `documentos-enviar-factura-senal-e3`, 6.4b): en una única operación atómica funde aprobar+enviar (espejo de E4/US-028). Ver §2.16 y DT-EMAIL-02 en §2.9. La FACTURA en `borrador` con `pdf_url` no nulo es el requisito previo.
...
#### Actualización de DT-EMAIL-02 (E3) — RESUELTA en US-023 (6.4b); condiciones movidas a E3 degradable en change `condiciones-particulares-senal-y-recordatorio-liquidacion`

El email E3 ha sido implementado en US-023 (change `documentos-enviar-factura-senal-e3`, 6.4b). El Gestor pulsa "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`): en una única operación atómica, aprueba la factura de señal (`borrador → enviada`) y envía E3 por `EnviarEmailPort` directo (rollback total si el proveedor falla) con la factura de señal y, si el tenant tiene condicions configuradas, el PDF de condicions particulars en blanco; crea COMUNICACION E3 `enviado`; registra AUDIT_LOG. Idempotente: 409 `E3_YA_ENVIADO` si ya existe COMUNICACION E3 `enviado`.

**Cambio del change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** las condicions particulars **se adjuntan ahora en E3** (factura de señal), no en E2 (presupuesto). El adjunto es **degradable** (`.catch(() => null)`): `EnviarFacturaSenalUseCase` llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` antes de la transacción; si el tenant no tiene condicions configuradas o el render falla, la URL es `null`, el adjunto se omite y la señal se envía igual **sin ningún 409**. Cuando la URL no es `null`, el adjunto se añade a E3 y, dentro de la transacción, se fijan `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` (`fijarCondicionesEnviadas`). El reenvío manual de E3 regenera el PDF en blanco vía `GenerarPdfCondicionesPort` (no busca un DOCUMENTO persistido). E2 (presupuesto) **ya no** invoca `GenerarPdfCondicionesPort` ni adjunta condicions particulars. Se eliminan la guarda dura `CONDICIONES_NO_CONFIGURADAS` (409), la clase `CondicionesNoConfiguradasError` y su fijación en `GenerarPresupuestoUseCase.confirmar()`. La factura de liquidación E4 incluye un párrafo recordatorio al cliente cuando `RESERVA.cond_part_firmadas = false`. `GenerarPdfCondicionesPort.generar()` sigue requiriendo el parámetro `idioma: 'es' | 'ca'` (la clave de storage incluye el idioma). Ver DT-EMAIL-02 en §2.9.
...
*Documento de arquitectura v5.4, 24/07/2026. Cambios respecto a v5.3: refleja el change `condiciones-particulares-senal-y-recordatorio-liquidacion` — actualiza §2.10 catálogo plantillas: E3 pasa a adjuntar condicions particulars de forma degradable (`.catch(() => null)`), fija `cond_part_enviadas_fecha`/`cond_part_firmadas` solo si adjunta, E2 adjunta solo el presupuesto, E4 incluye párrafo recordatorio condicional cuando `cond_part_firmadas = false`; actualiza DT-EMAIL-02 en §2.9 (E3 con adjunto degradable, elimina guard duro `CONDICIONES_NO_CONFIGURADAS` y `CondicionesNoConfiguradasError`, E4 con recordatorio condicional); actualiza §2.14 "Actualización DT-EMAIL-02 (E3)" con los mismos efectos.*

*Documento de arquitectura v5.3, 21/07/2026. Cambios respecto a v5.2: refleja el change `condiciones-idioma-e2-firma-banner` — actualiza §2.10 catálogo plantillas (E3 ya no lleva condicions particulars; se adjuntan en E2 desde la confirmación del presupuesto); actualiza DT-EMAIL-02 en §2.9 (E3 actualizada: condiciones movidas a E2, `GenerarPdfCondicionesPort` requiere `idioma`, `RESERVA.cond_part_enviadas_fecha`/`cond_part_firmadas` se fijan al confirmar presupuesto no al enviar E3, guard `CONDICIONES_NO_CONFIGURADAS` en `confirmar`); actualiza §2.14 nota "Actualización DT-EMAIL-02 (E3)" con los mismos efectos.*
...
*Documento de arquitectura v5.1, 15/07/2026. Cambios respecto a v5.0: refleja US-023 (change `documentos-enviar-factura-senal-e3`, 6.4b — Bloque C épico #6) — marca DT-EMAIL-02 E3 como RESUELTA (15/07/2026); actualiza §2.3 fila Email (E1 y E3 activas; E3 usa `EnviarEmailPort` directo, rollback atómico, espejo E4); actualiza §2.10 módulo Comunicaciones (E3 RESUELTA, comentario de cableado US-023, catálogo actualizado E3 activa); actualiza §2.14 (ciclo de vida FACTURA de señal: paso 3 — acción unificada aprobar+enviar US-023 con rollback total, efectos `cond_part_enviadas_fecha`/`cond_part_firmadas`, COMUNICACION E3 `enviado`, idempotencia 409 `E3_YA_ENVIADO`); añade nota de §2.14 "Actualización DT-EMAIL-02 (E3) RESUELTA"; sin migración de BD (campos `cond_part_*` y enum `E3` ya existían desde diseño inicial).*
