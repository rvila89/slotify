...
- **Coordinación con US-019 (D-6):** FIFO estricto + gana el primer lock. La guarda "ya promovida" — bajo `SELECT … FOR UPDATE` sobre RESERVA `2d` — es el mecanismo de coordinación entre la promoción automática y la futura manual. US-018 solo deja este contrato; la UI de US-019 y su mensaje de error ("La cola ya fue actualizada automáticamente") son de US-019.
