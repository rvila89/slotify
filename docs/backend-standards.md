...
| Email | Resend / Postmark (SDK) | Plantillas E1–E4, E6–E7, E9–E10 (E5/E8 eliminadas en change `fix-liquidacion-fianza-independientes`) |
...
| `comunicaciones` | Motor `DespacharEmailService` (application) + puertos `EnviarEmailPort` y `CatalogoPlantillasPort` (domain) + adaptadores `ResendEmailAdapter`/`FakeEmailAdapter` (infra, US-045) + catálogo de plantillas (infra); idempotencia por índice UNIQUE parcial `(reserva_id, codigo_email) WHERE reserva_id IS NOT NULL`; trazabilidad en `COMUNICACION` y `AUDIT_LOG`. **E1 y E3 activas**; E2, E4–E8 diseñadas/inactivas (cableado diferido). E3 usa `EnviarEmailPort` directo (sin motor) para garantizar rollback atómico si el proveedor falla (espejo de E4/US-028; 6.4b). Ver [architecture.md §2.10](./architecture.md) | UC-03, UC-19, UC-35, UC-36 |
