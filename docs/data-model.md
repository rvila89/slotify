...
**Divergencia intencional US-004 — validación de fecha (Gate 1, decisión A):** la ficha US-004 admitía `fecha_evento ≥ hoy` (incluyendo el día de hoy). La implementación adopta `fecha_evento > hoy` (estrictamente futura): el servidor rechaza `fecha_evento = hoy` y cualquier fecha pasada con error **400** sin crear `RESERVA` ni `FECHA_BLOQUEADA`. Motivo: alinear la rama de alta con fecha con la primitiva `validarFechaFutura` existente (US-040), el motor de tarifa (US-016) y el bloqueo atómico, manteniendo **una única regla de "fecha válida"** en todo el sistema. La UI impide seleccionar hoy y fechas pasadas (`min = mañana`). Aprobada explícitamente por el humano en el Gate 1. Trazabilidad: `openspec/changes/us-004-alta-consulta-con-fecha/design.md §D-1`; `openspec/changes/us-004-alta-consulta-con-fecha/specs/consultas/spec.md §Validación de fecha_evento estrictamente futura`.
...
**Reuso de `bloquearFecha()` en la misma transacción (US-004 / D-2):** el bloqueo blando de `2.b` se inserta en la **misma transacción** del alta (`UnidadDeTrabajoPrismaAdapter`), reutilizando `bloquearEnTx(tx, …)` — método interno de `FechaBloqueadaPrismaAdapter` que acepta el cliente transaccional Prisma. El método público `bloquear()` (US-040) permanece intacto como wrapper que abre su propia `$transaction` y delega en `bloquearEnTx`; el contrato externo de US-040 no cambia (cero regresión). Esto garantiza la atomicidad all-or-nothing `RESERVA 2b + FECHA_BLOQUEADA`: si el INSERT del bloqueo falla, la RESERVA tampoco se confirma. Fuente: `design.md §D-2`.
...
**Email E1 en el alta de consulta (UC-03 / US-003 / US-004):** toda alta genera un registro `COMUNICACION` con `codigo_email = 'E1'`. Sin `comentarios` → `estado = 'enviado'` y se invoca el `EnviarEmailPort` (adaptador stub no-op hasta US-045, que enchufará Resend/Postmark); con `comentarios` → `estado = 'borrador'`, el puerto **no** se invoca y la UI alerta al gestor sobre el borrador pendiente. En ambos casos la fila `COMUNICACION` se persiste dentro de la misma `$transaction` que crea la `RESERVA` y el `CLIENTE`. **Tarifa estimada en E1 (US-004 / D-4):** si el alta incluye `fecha_evento` + `num_adultos_ninos_mayores4` + `duracion_horas`, E1 incorpora la tarifa calculada vía `TarifaEstimadaPort` (motor UC-16); si falta algún dato o el cálculo no es posible (`TEMPORADA_NO_CONFIGURADA`, `TARIFA_NO_CONFIGURADA`, `tarifa_a_consultar = true`), E1 se envía con el **dossier general sin precio exacto**. La imposibilidad de calcular la tarifa **no bloquea el alta ni el bloqueo de fecha**. La tarifa estimada no se persiste en `RESERVA` (decorativa de E1). Fuente: `design.md §D-4`.
...
- **Fechas de evento:** `fecha_evento > hoy` (estrictamente futura, día natural) al crear una `Reserva` **con fecha** (alta vía `POST /reservas` con `fechaEvento`, US-004; y para toda operación de bloqueo vía `bloquearFecha()` / `validarFechaFutura`, US-040) y para el motor de tarifa (UC-16 / US-016). El servidor rechaza `fecha_evento = hoy` y fechas pasadas con **400** sin crear registros. **Divergencia intencional US-004 (Gate 1, decisión A):** la ficha US-004 admitía `≥ hoy`; se adoptó `> hoy` para mantener una única regla de fecha válida en todo el sistema, unificada con `validarFechaFutura` (US-040) y el motor de tarifa. Para consultas históricas, `fecha_evento` puede ser cualquier valor (ya ocurridas). La UI impide seleccionar hoy y fechas pasadas (`min = mañana`).
