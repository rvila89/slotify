...
| 16 | `Comunicacion` | `comunicacion` | Log de emails (E1–E4, E6–E7, E9–E10 + manuales) | UC-35, UC-36 |
...
| Campo | Tipo | Reglas / Notas |
|---|---|---|
| `condiciones` | `Json @default("{}")` | Contenido del PDF de condicions particulars. Estructura: `{ titulo: string; secciones: Array<{ titulo: string; cuerpo: string }> }`. Valor `{}` o `secciones` vacío = **tenant sin condicions configuradas**; `PdfCondicionesRealAdapter` devuelve `null`. **Desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** el adjunto de condicions particulars pasa de E2 (presupuesto) a **E3 (factura de señal)** de forma **degradable** (`.catch(() => null)`): si el tenant no tiene condicions configuradas o el render falla, la URL es `null`, el adjunto se omite y la señal se envía igual **sin ningún 409**. Se eliminan la guarda dura `CONDICIONES_NO_CONFIGURADAS` (409), la clase `CondicionesNoConfiguradasError` y la fijación de `cond_part_enviadas_fecha`/`cond_part_firmadas` dentro de `GenerarPresupuestoUseCase.confirmar()`. Anteriormente (`condiciones-idioma-e2-firma-banner`) el guard duro bloqueaba el presupuesto (E2); antes de ese change el guard estaba en E3. Añadido en 6.4a (migración `20260714130000_documento_condiciones_particulares`). Expuesto en el VO `ConfiguracionDocumentoTenant` como tipo `CondicionesDocumento`. |
...
> **Migración (change `fix-liquidacion-fianza-independientes`):** el campo `iban_devolucion` ha sido eliminado del modelo junto con los emails E5/E8 y la captura de IBAN al cliente. La devolución de fianza ya no requiere IBAN: se registra directamente mediante el botón "Devolver fianza" en `post_evento` (UC-27).
...
| `fianza_status` | `FianzaStatus @default(pendiente)` | `pendiente \| cobrada \| devuelta`. `cobrada` = comprobante de transferencia recibido subido por el gestor (change `fix-liquidacion-fianza-independientes`; se eliminan `recibo_enviado` y `retenida_parcial`). |
...
| `fianza_eur` | `Decimal? @db.Decimal(10,2)` | Importe de fianza configurado en `TENANT_SETTINGS` en el momento de la confirmación |
| `fianza_comprobante_fecha` | `DateTime?` | Fecha en que el gestor subió el comprobante de transferencia de fianza recibida. Análogo a `cond_part_firmadas_fecha`. `NULL` si no se ha subido comprobante. (Añadido en change `fix-liquidacion-fianza-independientes`.) |
| `fianza_cobrada_fecha` | `DateTime?` | |
| `fianza_devuelta_fecha` | `DateTime?` | Fecha de la devolución completa de la fianza |
...
| `cond_part_firmadas` | `Boolean @default(false)` | `false` cuando el cliente no ha devuelto el contrato firmado. **Desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** se fija a `false` dentro de la transacción de emisión de E3 (`POST /reservas/{id}/facturas/senal/enviar`, US-023) **solo si** el adjunto de condicions particulars fue efectivamente generado (adjunto no degradado); si el tenant no tiene condicions configuradas, el campo no se modifica. A `true` al registrar la firma (UC-19 flujo registro, US-024). El valor de este campo governa el párrafo recordatorio de E4 (liquidación): si `false`, E4 incluye el recordatorio al cliente de que las condicions particulars están pendientes de devolver firmadas. Una alerta no bloqueante A29 se emite en T-0 si sigue en `false` (US-031). Anteriormente (`condiciones-idioma-e2-firma-banner`) se fijaba a `false` al confirmar el presupuesto (E2/UC-14). |
| `cond_part_enviadas_fecha` | `DateTime?` | Timestamp en que las condicions particulars fueron enviadas al cliente. **Desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** fijado al enviar E3 (`POST /reservas/{id}/facturas/senal/enviar`, US-023) **solo si** el PDF de condicions fue adjuntado (adjunto no degradado); si el tenant no tiene condicions configuradas, permanece `null`. Se actualiza al nuevo timestamp en cada reenvío manual (`POST /reservas/{id}/facturas/senal/reenviar`) que adjunte condicions. `null` si no se han enviado condicions todavía. La precondición de US-024 (registro de firma) sigue siendo que este campo no sea `null`. Anteriormente (`condiciones-idioma-e2-firma-banner`) se fijaba al confirmar el presupuesto (E2/UC-14). |
...
**Cambio atómico de fecha con salida de cola (`2d → 2b`) (change `cambiar-fecha-consulta-en-cola` — sin migración):** el Gestor cambia la fecha de una RESERVA en `sub_estado = '2d'` (cola de espera) vía `POST /reservas/{id}/cambiar-fecha`. Esta rama es un **segundo origen** del endpoint, independiente de la rama `2b`/`2c`/`2v`. La guarda declarativa **separada** `esOrigenCambiarFechaEnCola` (`ORIGENES_CAMBIAR_FECHA_EN_COLA = [{ estado: 'consulta', subEstado: '2d' }]`) en `maquina-estados.ts` selecciona esta rama. La RESERVA en `2d` **no posee** fila en `FECHA_BLOQUEADA` (está en cola, no bloquea nada). Si la `fecha_nueva` está **libre**: en una única transacción all-or-nothing bajo RLS del tenant con `SELECT … FOR UPDATE` sobre la RESERVA y sobre `FECHA_BLOQUEADA(tenant_id, fecha_nueva)`: (1) INSERTA un bloqueo blando de `fecha_nueva` (`bloquearEnTx` / `resolverPlanBloqueo` fase `2.b`, `tipo_bloqueo='blando'`, `ttl_expiracion = now() + ttl_consulta_dias`); (2) UPDATE `RESERVA.fecha_evento = fecha_nueva`; (3) cambia `sub_estado = '2b'`; (4) `posicion_cola → NULL`, `consulta_bloqueante_id → NULL` (salida de la cola); (5) **reordena la cola vieja** decrementando en 1 la `posicion_cola` de todas las RESERVA en `2d` con el mismo `consulta_bloqueante_id` y `posicion_cola > P`, preservando `UNIQUE(tenant_id, consulta_bloqueante_id, posicion_cola) WHERE posicion_cola IS NOT NULL`; (6) crea una `COMUNICACION` E1 en `borrador` (`fecha_envio = NULL`, **no autoenviada**) reutilizando `plantilla-transicion-fecha.ts` rama `'disponible'`; (7) `AUDIT_LOG accion='actualizar'`, `entidad='RESERVA'`, con los cambios de `sub_estado`, `posicion_cola` y `consulta_bloqueante_id` en `datos_nuevos`. El sistema **NO promueve** ninguna cola (la RESERVA en `2d` no libera bloqueo) y **NO modifica** la RESERVA bloqueante de su fecha antigua. Si `fecha_nueva` está **ocupada**: aborta con **409 terminal** (rollback total); la RESERVA conserva `sub_estado='2d'`, `posicion_cola` y `consulta_bloqueante_id`; **no se ofrece re-encolar** (el error expone solo `motivo`, sin `colaDisponible`). **Sin migración de BD**: las columnas `posicion_cola`, `consulta_bloqueante_id`, `ttl_expiracion` y `sub_estado` ya existen desde US-000/US-004. Ver `er-diagram.md §3.7` y `specs/consultas/spec.md` para la especificación técnica completa.
...
- **Sin email**: la transición a `2z` no activa ningún código del catálogo de comunicaciones.
...
3. Post-commit (solo si `enviar=true`): regeneración del PDF + UPDATE `pdf_url` en la fila del PRESUPUESTO (best-effort, patrón `guardarPdfUrl` de facturación; permite que futuros reenvíos reutilicen el PDF sin regenerarlo); disparo de E2 via `DespacharEmailService.despacharReenvio` (salta la idempotencia del camino `despachar`; la variable `esEdicion=true` modifica asunto y párrafo inicial de la plantilla E2/E2-CA); el motor escribe la **única** `COMUNICACION` con `codigo_email='E2'`, `es_reenvio=true`, `estado ∈ {'enviado','fallido'}` — fuente única post-commit, sin fila contable adicional dentro de la transacción.
4. Reenvío sin cambios (`POST /reservas/{id}/presupuesto/reenvio`): NO crea versión nueva; usa el `PRESUPUESTO.pdf_url` vigente como adjunto (regenera el PDF si `pdf_url` es `null`, p. ej. en presupuestos históricos previos a este change); disparo de E2 via `DespacharEmailService.despacharReenvio` con `esEdicion=false` (texto E2 estándar, sin marca de edición); el motor escribe la única `COMUNICACION E2 es_reenvio=true` + `AUDIT_LOG`; responde `200`.
...
### 3.12 Factura
Facturas de señal (40%), liquidación (60% + extras) y complementarias. **La fianza ya no es una FACTURA emitida** (change `fix-liquidacion-fianza-independientes`): pasa a un flujo pasivo de comprobante (ver §3.15 `tipo = comprobante_fianza`).
...
| `tenant_id` | `String` | FK → `Tenant` |
| `reserva_id` | `String` | FK → `Reserva` |
| `numero_factura` | `String?` | Secuencial `F-2026-0001`. **Nullable en borradores** (US-027): el borrador de liquidación se crea con `numero_factura = NULL`; la numeración se asigna al emitir. El constraint `UNIQUE(tenant_id, numero_factura)` solo aplica a valores no nulos. Migración: `20260704130000_us027_numero_factura_nullable` (DROP NOT NULL, aditiva). |
| `tipo` | `TipoFactura` | `senal \| liquidacion \| complementaria`. El valor `fianza` ha sido eliminado (change `fix-liquidacion-fianza-independientes`). |
...
Al activarse los sub-procesos paralelos de una RESERVA que ha transitado a `reserva_confirmada` (efecto post-commit de US-021, espejo del disparo de la factura de señal de US-022), se genera el borrador de liquidación; su fallo **no revierte** la confirmación ya realizada y la operación es reintentable por idempotencia. **Ya no se genera borrador ni recibo de fianza**.

- **Factura de liquidación**: `tipo = 'liquidacion'`, `estado = 'borrador'`, `numero_factura = NULL`, `total = RESERVA.importe_liquidacion + Σ(RESERVA_EXTRA.subtotal WHERE factura_id IS NULL)`. El `importe_liquidacion` viene **congelado** de US-021 (60 % MVP); no se recalcula porcentaje ni tarifa. Los `subtotal` ya congelados por línea se suman sin recalcular. **No se marca** `factura_id` en `RESERVA_EXTRA` mientras el documento está en borrador: ese marcado ocurre al emitir. Desglose fiscal: `base_imponible = round(total / 1,21, 2)`, `iva_importe = total − base_imponible`.
- **Idempotencia**: guarda de existencia por `(reserva_id, tipo)` + constraint `UNIQUE(reserva_id, tipo)` (US-022). El tipo `fianza` ya no existe en el enum.
...
### 3.13 Pago
Cobro conciliado contra una factura. El justificante es un `Documento(tipo=justificante_pago)`. Materializado en US-029 mediante migración aditiva `20260704150000_us029_pago_tenant_id`. **La fianza ya no usa PAGO** (change `fix-liquidacion-fianza-independientes`): su cobro se registra mediante comprobante de transferencia (`DOCUMENTO(tipo=comprobante_fianza)`), no como PAGO contra una FACTURA.
...
| `fecha_cobro` | `DateTime @db.Date` | Fecha del cobro. Para la **liquidación** (US-029), `fecha_cobro ≤ hoy` (no futura). |
...
**`liquidacion_status = cobrada` como precondición de `evento_en_curso` (US-031):** al completar el cobro, `RESERVA.liquidacion_status = 'cobrada'` habilita una de las **dos** precondiciones de la transición `reserva_confirmada → evento_en_curso` (junto con `pre_evento_status = cerrado`). La fianza ya no es una precondición (change `fix-liquidacion-fianza-independientes`). `RESERVA.estado` permanece `reserva_confirmada` tras el cobro; la transición a `evento_en_curso` es responsabilidad de US-031.
...
| `tipo` | `TipoDocumento` | `dni_anverso \| dni_reverso \| clausula_responsabilidad \| condiciones_particulares \| justificante_pago \| presupuesto \| factura \| comprobante_fianza \| otro`. El valor `comprobante_fianza` se añade en el change `fix-liquidacion-fianza-independientes` para el comprobante de transferencia de fianza recibida subido por el gestor. |
...
### 3.16 Comunicacion
Log de emails enviados (E1–E7, E9, E10 + manuales). **E5 y E8 han sido eliminados** en el change `fix-liquidacion-fianza-independientes`. **E10** (fianza devuelta, CA/ES, activo) es el código nuevo. El motor hexagonal `DespacharEmailService` (US-045) es el único responsable de registrar y actualizar las entradas de esta entidad para los emails del ciclo de vida. La primera superficie HTTP del módulo (US-046) expone las acciones manuales del Gestor (listar, enviar borrador, descartar, email manual) como sub-recurso de la RESERVA.
...
| `codigo_email` | `CodigoEmail` | `E1 \| E2 \| E3 \| E4 \| E6 \| E7 \| E9 \| E10 \| manual`. E5 y E8 eliminados (change `fix-liquidacion-fianza-independientes`); E10 = "fianza devuelta", activo, bilingüe CA/ES. |
| `subtipo` | `SubtipoEmail?` | Enum nullable. Distingue el evento del ciclo de vida que generó el email E1. Valores: `consulta_exploratoria`, `fecha_disponible`, `fecha_confirmada`, `cola_espera`, `cambio_fecha`, `solicitud_datos`. `NULL` para E2–E4, E6–E7, E9–E10, emails `manual` y filas legadas. Añadido en el change `historial-completo-comunicaciones`; `solicitud_datos` añadido en el change `solicitud-datos-presupuesto-borrador`. |
...
**Idempotencia (US-045 / US-028 / US-046 / change `historial-completo-comunicaciones` — migración `20260628120000_us045_comunicacion_idempotencia_indice`, actualización D-indice-terna):** índice UNIQUE parcial `uq_comunicacion_reserva_codigo` clavado sobre la **terna** `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `WHERE reserva_id IS NOT NULL AND es_reenvio = false AND codigo_email <> 'manual' AND estado = 'enviado'`. Semántica del nuevo índice:
- **Historial completo de borradores:** el predicado incluye `estado = 'enviado'`; los `borrador` (de cualquier subtipo) no están sujetos al constraint y coexisten libremente.
- **Subtipos distintos pueden ambos estar `enviado`:** dos filas E1 con `subtipo` diferente son emails semánticamente distintos, **no** reenvíos; el constraint no colisiona.
- **Solo una segunda fila `enviado` de la misma terna colisiona:** ese sí es un reenvío genuino → `es_reenvio = true` (patrón E3/E4), queda fuera del predicado.
- **`NULLS NOT DISTINCT`:** preserva la idempotencia de E2–E4, E6–E7, E9–E10 (`subtipo = NULL`): dos intentos de enviar el mismo E2 siguen colisionando.
- Reenvíos (`es_reenvio = true`) y emails `manual` siguen fuera del constraint.
El motor consulta la existencia antes de insertar filtrando `estado = 'enviado'`; el índice actúa como backstop ante carreras concurrentes de doble envío idéntico (violación → `ComunicacionDuplicadaError`, no 500). La migración D-4 de US-028 añadió `AND es_reenvio = false`; la D-5 de US-046 añadió `AND codigo_email <> 'manual'`; el change `historial-completo-comunicaciones` amplía la clave a la terna y añade `AND estado = 'enviado'`. Ver también §5.

**Email E1 en el alta de consulta (UC-03 / US-003/004 + US-045 motor real):** el alta crea siempre la `COMUNICACION` E1 dentro de la `$transaction` con `estado = 'borrador'` (estado no final, sin `fecha_envio`) y `subtipo` derivado del campo `tipoE1`: `sin_fecha → consulta_exploratoria`; `fecha_disponible → fecha_disponible`; `fecha_confirmada → fecha_confirmada`; `fecha_cola → cola_espera`. Post-commit, el motor `DespacharEmailService.finalizarEnvio` la promueve:
...
**Email E6 en la transición a 2.v (UC-07 / US-008 / D-6):** tras el COMMIT de la transición `{2a|2b|2c} → 2v` (RESERVA + FECHA_BLOQUEADA + AUDIT_LOG all-or-nothing), el motor `DespacharEmailService` (US-045) dispara el envío del email **E6** (confirmación de visita programada con fecha y hora) al cliente y registra en `COMUNICACION` con `codigo_email = 'E6'`, `estado = 'enviado'`, `reserva_id`, `cliente_id` y `tenant_id`. El disparo es **posterior al commit** y no bloqueante: un fallo del proveedor no revierte el `sub_estado = '2v'` ni el bloqueo ya comprometido; el fallo queda trazado en `COMUNICACION` (estado distinto de `'enviado'`) para seguimiento/reintento, coherente con el patrón de US-045. E6 se registra en todos los casos de transición exitosa, independientemente de si el bloqueo fue INSERTado (origen `2a`) o UPDATEado (origen `2b`/`2c`). En test/CI el transporte opera en **modo fake** (sin envíos reales). La idempotencia del índice UNIQUE parcial sobre la terna `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `estado='enviado'` impide duplicar la fila E6 (`subtipo=NULL`) ante retries. Fuente: `design.md §D-6`; `specs/comunicaciones/spec.md`.

**Email E2 en la confirmación de presupuesto / activación de pre-reserva (UC-14 / US-014):** tras el COMMIT de la transacción `{2a|2b|2c|2v} → pre_reserva` (PRESUPUESTO + RESERVA + FECHA_BLOQUEADA + cola A16 + AUDIT_LOG all-or-nothing), el motor `DespacharEmailService` (US-045) dispara el envío del email **E2** (presupuesto formal con PDF adjunto) al cliente via el camino idempotente `despachar` (`es_reenvio=false`) y registra en `COMUNICACION` con `codigo_email = 'E2'`, `estado = 'enviado'`, `reserva_id`, `cliente_id`. El PDF se genera post-commit mediante el puerto de PDF (react-pdf; modo fake en dev/CI, `pdf_url = null`); una vez generado, se actualiza `PRESUPUESTO.pdf_url` y se referencia en E2. El disparo es **posterior al commit** y no bloqueante: un fallo del proveedor de email o de generación del PDF no revierte la `pre_reserva` ya comprometida; el fallo queda trazado en `COMUNICACION` para seguimiento. La idempotencia del índice UNIQUE parcial sobre la terna `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `estado='enviado'` impide duplicar la fila E2 (`subtipo=NULL`) ante retries. Fuente: `design.md §D-6`; UC-14.

**Email E2 en la edición y el reenvío sin cambios del presupuesto (UC-15 / US-015 — change `presupuesto-edicion-reenvio-email-real`):** tanto la edición con envío explícito (`POST /presupuesto/edicion` con `enviar=true`) como el reenvío sin cambios (`POST /presupuesto/reenvio`) invocan el motor via **`DespacharEmailService.despacharReenvio`**, que **salta la idempotencia** del camino `despachar` (el cual, al encontrar el E2 original con `es_reenvio=false`, devolvía `motivo='idempotente'` y no invocaba al proveedor). `despacharReenvio` crea la **única** `COMUNICACION` con `codigo_email='E2'`, `es_reenvio=true` y `estado ∈ {'enviado','fallido'}` post-commit — no se escribe ninguna fila contable adicional dentro de la transacción (fuente única = el motor). En la edición, la variable `esEdicion=true` (derivada en servidor, no entra por el contrato) modifica el asunto («Hemos actualizado tu presupuesto…») y antepone un párrafo de "presupuesto actualizado" en las plantillas `renderE2`/`renderE2Ca`; en el reenvío sin cambios `esEdicion=false` (texto E2 estándar). El reenvío sin cambios usa el `PRESUPUESTO.pdf_url` de la versión vigente como adjunto, que ahora se persiste post-commit tanto en el flujo de generación (UC-14) como en el de edición (UC-15). El fallo del proveedor deja `COMUNICACION.estado='fallido'` sin revertir la versión del PRESUPUESTO ya comprometida (best-effort post-commit). Fuente: `design.md §D1/D2`; `specs/presupuestos/spec.md` (MODIFIED/ADDED).

**Borrador E1 en la transición 2.a → 2.b/2.d (UC-04 / US-005 / change `email-transicion-fecha-borrador`):** dentro de la misma transacción de la transición (no post-commit), el sistema crea la `COMUNICACION` E1 con `estado='borrador'`, `fecha_envio=null`, `subtipo` según la rama: `fecha_disponible` (rama libre `2a → 2b`) o `cola_espera` (rama cola `2a → 2d`, gestor acepta). **Sin auto-envío.** El contenido se genera mediante `renderMensajeTransicionFecha` (módulo de aplicación puro, sin `DespacharEmailService`): rama libre → plantilla dinámica "disponible" (bilingüe `ca`/castellano); rama cola → plantilla dinámica "cola" (bilingüe). La `COMUNICACION` queda en `borrador` para revisión/envío manual del gestor (flujo US-046). El índice UNIQUE parcial sobre la terna `(reserva_id, codigo_email, subtipo)` con predicado `estado = 'enviado'` impide duplicados de envíos consumados. Sin migración de columnas (la columna `subtipo` se añade en el change `historial-completo-comunicaciones`). Fuente: `design.md §D-6`; change `email-transicion-fecha-borrador`.

**Borrador E1 en el cambio de fecha de una consulta 2.b (change `historial-completo-comunicaciones`):** cambiar la fecha de una consulta ya en sub-estado `2b` (rama no-cola de `cambiar-fecha.use-case.ts`) ahora **emite un borrador E1** (`subtipo = 'cambio_fecha'`) dentro de la misma transacción. Antes esta rama no producía ninguna `COMUNICACION`. La rama de salida de cola (2d → 2b, `cambiarDesdeCola`) emite `subtipo = 'fecha_disponible'`. El adaptador `cambiar-fecha-uow.prisma.adapter.ts` persiste el `subtipo` que el caso de uso le pasa en `CrearBorradorE1Params`. Ambos son siempre `borrador` (sin auto-envío); el gestor los revisa y envía desde US-046. Fuente: `design.md §D-insert-no-upsert`; change `historial-completo-comunicaciones`.

**Borrador E1 de solicitud de datos fiscales (change `solicitud-datos-presupuesto-borrador`):** cuando el gestor intenta generar el presupuesto (UC-14) y los datos fiscales del cliente están incompletos, puede optar por solicitar al cliente que los complete directamente. El endpoint `POST /reservas/{id}/comunicaciones/solicitar-datos-presupuesto` crea una `COMUNICACION` E1 con `subtipo = 'solicitud_datos'` y `estado = 'borrador'`, redactada en el idioma de `RESERVA.idioma` (`es`/`ca`), que solicita al cliente nombre y apellidos, DNI y dirección/población. El borrador reutiliza la plantilla del E1 "disponible". **Idempotencia:** 409 si ya existe una `COMUNICACION` E1 con `subtipo = 'solicitud_datos'` y `estado = 'enviado'` para esa reserva (el índice UNIQUE parcial sobre la terna lo garantiza); reutiliza el borrador pendiente si existe en `estado = 'borrador'`; 422 si los datos fiscales ya están completos; 404 si la reserva no existe. El borrador se revisa y envía mediante el flujo US-046 (UC-36). Migración aditiva: `20260724120000_subtipo_solicitud_datos`.

**Email E8 en el registro del IBAN de devolución (UC-26 FA-01 / UC-27 pasos 1–3 / US-035):** tras el COMMIT de la transacción de escritura del IBAN (`UPDATE CLIENTE.iban_devolucion` + `INSERT AUDIT_LOG`), el motor `DespacharEmailService` (US-045) despacha el email **E8** (confirmación de recepción del IBAN + próximos pasos para la devolución de la fianza) al `CLIENTE.email` — nunca al gestor — y registra en `COMUNICACION` con `codigo_email = 'E8'`, `estado = 'enviado'`, `fecha_envio`, `reserva_id`, `cliente_id` y `tenant_id`. El disparo es **posterior al commit y no bloqueante** (patrón guardar-luego-enviar, `design.md §D-2`): un fallo del proveedor no revierte el IBAN ya persistido; `COMUNICACION.estado = 'fallido'` sin `fecha_envio`, y el gestor recibe alerta para reintentar desde la ficha. **Excepción auditada a la idempotencia (D-3A):** el índice UNIQUE parcial sobre la terna `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `estado='enviado'` protege contra duplicados por disparos automáticos de E8 (`subtipo=NULL`), pero cada registro/corrección intencional del IBAN por el gestor inserta una **nueva** `COMUNICACION` E8 con `es_reenvio = true` (igual que el reenvío de E4 en US-028, `design.md §D-3`): al marcarse `es_reenvio = true` la fila queda fuera del predicado del índice y el constraint no aplica. Esto garantiza trazabilidad completa de cada confirmación enviada con cada versión del IBAN. En test/CI el transporte opera en **modo fake** (sin envíos reales). Fuente: `design.md §D-2..D-3`; `specs/comunicaciones/spec.md`; UC-26/UC-27.
...
- `reserva_id` no nulo para E1–E4, E6–E7, E9–E10 y para emails `manual` creados desde la ficha de una RESERVA (US-046); solo nullable para emails `manual` creados fuera del contexto de una reserva.
...
- `subtipo` no nulo para todas las filas E1 (valor derivado del evento del ciclo de vida); `NULL` para E2–E4, E6–E7, E9–E10, emails `manual` y filas legadas anteriores al change `historial-completo-comunicaciones`.
...
| `UNIQUE PARTIAL (reserva_id, codigo_email, subtipo) NULLS NOT DISTINCT WHERE reserva_id IS NOT NULL AND es_reenvio = false AND codigo_email <> 'manual' AND estado = 'enviado'` | `Comunicacion` | Idempotencia del motor de email clavada sobre la **terna** (US-045, D-4 US-028, D-5 US-046, D-indice-terna change `historial-completo-comunicaciones`). Solo aplica a filas `estado='enviado'`: borradores coexisten libremente, subtipos distintos pueden ambos estar enviados. Solo colisiona un segundo `enviado` de la misma terna (reenvío genuino → `es_reenvio=true`, fuera del predicado). `NULLS NOT DISTINCT` preserva la idempotencia de E2–E8 (`subtipo NULL`). Reenvíos y `manual` fuera del constraint. Migración `20260628120000_us045_comunicacion_idempotencia_indice`; ampliado por D-4 US-028, D-5 US-046 y change `historial-completo-comunicaciones`. |
...
*Documento de modelo de datos v3.2 (21/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v4.13. v3.2: refleja change `fix-importe-total-confirmar-senal` — corrige y completa la documentación del campo `RESERVA.importe_total` en §3.5 (se congela en UC-17/US-021, no en UC-14; el sistema toma `PRESUPUESTO.total` del presupuesto vigente `MAX(version)` con `estado='enviado'`, valida `> 0` y lo escribe en la transacción atómica; permanece NULL hasta esa transición); corrige el bullet `aceptado` del mapa de estados del PRESUPUESTO en §3.11 (productor es UC-17/US-021, no un aceptado explícito del cliente; dentro de la misma transacción se marca el presupuesto vigente como `aceptado` y se congela `importe_total`). Sin migración de BD, sin cambio de contrato OpenAPI.*

*Documento de modelo de datos v3.1 (20/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v4.11. v3.1: refleja change `historial-completo-comunicaciones` — añade campo `subtipo SubtipoEmail?` en la tabla de §3.16 Comunicacion (enum nullable; `NULL` para E2–E8, `manual` y filas legadas; valores `consulta_exploratoria`, `fecha_disponible`, `fecha_confirmada`, `cola_espera`, `cambio_fecha`); actualiza la nota de idempotencia a la terna `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `estado='enviado'`; añade nota de `subtipo` en el alta de E1 (mapeo `tipoE1`); actualiza las notas de borrador E1 en la transición `2a→2b/2d` (añade `subtipo`) y añade nota de borrador E1 en cambio de fecha de `2b` (nuevo comportamiento: `subtipo='cambio_fecha'`); añade regla de validación de `subtipo`; actualiza la entrada del índice UNIQUE parcial en §5 a la nueva clave de terna.*
...
*Documento de modelo de datos v2.8 (24/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v5.4. v2.8: refleja change `condiciones-particulares-senal-y-recordatorio-liquidacion` — actualiza §3.2b campo `condiciones` de `PLANTILLA_DOCUMENTO_TENANT` (el adjunto de condicions pasa de E2 con guard duro a E3 degradable; se eliminan la guarda dura `CONDICIONES_NO_CONFIGURADAS` y `CondicionesNoConfiguradasError`); actualiza §3.5 campos `cond_part_firmadas` y `cond_part_enviadas_fecha` de RESERVA (se fijan en E3 solo si el PDF fue adjuntado, no al confirmar el presupuesto; `cond_part_firmadas` governa el párrafo recordatorio de E4). Sin entidades, columnas ni índices nuevos.*

*Documento de modelo de datos v2.7 (21/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v5.3. v2.7: refleja change `condiciones-idioma-e2-firma-banner` — actualiza §3.3 campo `condiciones` de `PLANTILLA_DOCUMENTO_TENANT` (guard duro de `null` pasa a estar en E2 al confirmar presupuesto, no en E3; E3 ya no adjunta condicions particulars); actualiza §3.5 campos `cond_part_firmadas` y `cond_part_enviadas_fecha` de RESERVA (ambos se fijan al confirmar el presupuesto en E2/UC-14, no al enviar E3/US-023; `GenerarPdfCondicionesPort.generar()` ahora requiere `idioma: 'es' | 'ca'`; clave storage `condiciones/{tenantId}-{idioma}.pdf`). Sin entidades, columnas ni índices nuevos.*

*Documento de modelo de datos v2.6 (18/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v4.2. v2.6: refleja el change `email-transicion-fecha-borrador` (US-005 / UC-04) — actualiza la nota de §3.16 Comunicacion "Extensión de E1 en la transición 2.a → 2.b" renombrándola a "Borrador E1 en la transición 2.a → 2.b/2.d": la COMUNICACION E1 ya NO es un UPSERT post-commit enviado por el motor `DespacharEmailService`/`ConfirmacionBloqueoEmailAdapter`; ahora se crea como INSERT `estado='borrador'` dentro de la misma transacción, mediante `renderMensajeTransicionFecha` (módulo de aplicación puro, bilingüe, con plantillas dinámicas "disponible"/"cola"), sin auto-envío. La rama cola (`2a → 2d`) también genera borrador E1. Sin migración de columnas.*
