...
*Documento de modelo de datos v1.9 (01/07/2026). Derivado y consistente con [er-diagram.md](./er-diagram.md) v3.0. v1.9: refleja US-018 — promoción automática de cola (UC-12): sin columnas ni índices nuevos; el modelo ya tenía `posicion_cola`, `consulta_bloqueante_id` y `ttl_expiracion`; la nota de `liberarFecha()` en §3.6 (remite a er-diagram.md §3.6) y la nota de la cola en er-diagram.md §5.2 reflejan el adaptador real. Ver [er-diagram.md](./er-diagram.md) v3.0 para el detalle.*
