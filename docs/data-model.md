...
| Campo | Tipo | Reglas / Notas |
|---|---|---|
| `condiciones` | `Json @default("{}")` | Contenido del PDF de condicions particulars. Estructura: `{ titulo: string; secciones: Array<{ titulo: string; cuerpo: string }> }`. Valor `{}` o `secciones` vacío = **tenant sin condicions configuradas**; `PdfCondicionesRealAdapter` devuelve `null`. **Desde el change `condiciones-idioma-e2-firma-banner`:** la devolución de `null` es un **requisito bloqueante en E2** — al confirmar el presupuesto (`POST /reservas/{id}/presupuesto`, `GenerarPresupuestoUseCase.confirmar()`), la guarda pre-transacción llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` antes de abrir la transacción; si devuelve `null` → 409 `CONDICIONES_NO_CONFIGURADAS` sin ningún efecto. El PDF de condiciones se adjunta a E2 (con el presupuesto). E3 ya no adjunta condicions particulars. Anteriormente (hasta change `condiciones-idioma-e2-firma-banner`) el guard estaba en E3 y E2 lo omitía sin error. Añadido en 6.4a (migración `20260714130000_documento_condiciones_particulares`). Expuesto en el VO `ConfiguracionDocumentoTenant` como tipo `CondicionesDocumento`. |
...
| `cond_part_firmadas` | `Boolean @default(false)` | `false` cuando el cliente no ha devuelto el contrato firmado. **Desde el change `condiciones-idioma-e2-firma-banner`:** se fija a `false` al confirmar el presupuesto en E2 (`POST /reservas/{id}/presupuesto`, UC-14) y a `true` al registrar la firma (UC-19 flujo registro). Una alerta no bloqueante A29 se emite en T-0 si sigue en `false` (US-031). Anteriormente (hasta change `condiciones-idioma-e2-firma-banner`) se fijaba a `false` al enviar E3 (US-023). |
| `cond_part_enviadas_fecha` | `DateTime?` | Timestamp en que las condicions particulars fueron enviadas al cliente. **Desde el change `condiciones-idioma-e2-firma-banner`:** fijado en la confirmación del presupuesto (`POST /reservas/{id}/presupuesto`, UC-14, E2). `null` si no se ha confirmado el presupuesto todavía. Anteriormente (hasta change `condiciones-idioma-e2-firma-banner`) se fijaba al enviar E3 (`POST /reservas/{id}/facturas/senal/enviar`, US-023) y se actualizaba en cada reenvío manual (`POST /reservas/{id}/facturas/senal/reenviar`). |
