...
**Borrador E1 en la transición 2.a → 2.b/2.d (UC-04 / US-005 / change `email-transicion-fecha-borrador`):** dentro de la misma transacción de la transición (no post-commit), el sistema crea la `COMUNICACION` E1 con `estado='borrador'`, `fecha_envio=null`, **sin auto-envío**. El contenido se genera mediante `renderMensajeTransicionFecha` (módulo de aplicación puro, sin `DespacharEmailService` ni `ConfirmacionBloqueoEmailAdapter`): rama libre (`2a → 2b`) → plantilla dinámica "disponible" (bilingüe `ca`/castellano; variables: nombre del cliente, fecha del evento, personas, horas; placeholder `___` si faltan); rama cola (`2a → 2d`, gestor acepta) → plantilla dinámica "cola" (bilingüe; variables: nombre, fecha). La `COMUNICACION` queda en `borrador` para revisión/envío manual del gestor (flujo US-046). El índice UNIQUE parcial `(reserva_id, codigo_email) WHERE reserva_id IS NOT NULL AND es_reenvio = false AND codigo_email <> 'manual'` garantiza una sola fila E1 no-reenvío por reserva. Sin migración (columnas ya existentes). Fuente: `design.md §D-6`; change `email-transicion-fecha-borrador`.
