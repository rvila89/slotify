...
| `subtipo` | `SubtipoEmail?` | Enum nullable. Distingue el evento del ciclo de vida que generó el email E1. Valores: `consulta_exploratoria`, `fecha_disponible`, `fecha_confirmada`, `cola_espera`, `cambio_fecha`, `solicitud_datos`. `NULL` para E2–E8, emails `manual` y filas legadas. Añadido en el change `historial-completo-comunicaciones`; `solicitud_datos` añadido en el change `solicitud-datos-presupuesto-borrador`. |
...
- **Solo una segunda fila `enviado` de la misma terna colisiona:** ese sí es un reenvío genuino → `es_reenvio = true` (patrón E3/E4/E8), queda fuera del predicado.
- **`NULLS NOT DISTINCT`:** preserva la idempotencia de E2–E8 (`subtipo = NULL`): dos intentos de enviar el mismo E2 siguen colisionando.
...
- `subtipo` no nulo para todas las filas E1 (valor derivado del evento del ciclo de vida); `NULL` para E2–E8, emails `manual` y filas legadas anteriores al change `historial-completo-comunicaciones`.
