...
**Divergencia intencional US-004 — regla de fecha (Gate 1, decisión A):** la validación previa a la transacción exige `fecha_evento > hoy` (estrictamente futura) para el alta con fecha y para toda operación de bloqueo. La ficha US-004 admitía `≥ hoy`; se adoptó `> hoy` para mantener **una única regla de "fecha válida"** en todo el sistema, unificada con `validarFechaFutura` (US-040) y el motor UC-16. El servidor rechaza `fecha_evento = hoy` y fechas pasadas con **400** sin crear `RESERVA` ni `FECHA_BLOQUEADA`. La UI impide seleccionar hoy y fechas pasadas (`min = mañana`). Aprobada por el humano en el Gate 1. Trazabilidad: `design.md §D-1`, `specs/consultas/spec.md §Validación de fecha_evento estrictamente futura`.
