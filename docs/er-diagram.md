...
**Nota US-051 — operación atómica "cambiar fecha ya bloqueada" (`POST /reservas/{id}/cambiar-fecha`):** cuando la RESERVA tiene ya una fecha bloqueada (`sub_estado ∈ {2b, 2c, 2v}`), el cambio de fecha no puede hacerse por el PATCH genérico (que no toca `fechaEvento`). Existe un endpoint dedicado que realiza, en una **única transacción** con `SELECT … FOR UPDATE`, las siguientes operaciones atómicas sobre `FECHA_BLOQUEADA`:

1. Adquiere `FOR UPDATE` sobre la RESERVA y sobre la fila `FECHA_BLOQUEADA(tenant_id, fecha_nueva)`.
2. Si `fecha_nueva` está libre: `bloquearFecha(tenant_id, fecha_nueva)` (INSERT nueva fila) + UPDATE `RESERVA.fecha_evento` + `liberarFecha(tenant_id, fecha_antigua)` (DELETE de la fila antigua) + si la fecha antigua tenía cola activa, disparo de `PromocionColaPort` (mecánica A15, FIFO).
3. Si `fecha_nueva` ya está bloqueada por otra RESERVA: la transacción aborta con 409 y la fecha antigua permanece intacta (rollback total).
4. `AUDIT_LOG` `accion='actualizar'`, `entidad='RESERVA'`, con la fecha anterior y la nueva.

Esta operación reutiliza `bloquearFecha()` y `liberarFecha()` (UC-30/UC-31 / US-040/US-041) como primitivas; no introduce locks distribuidos. La garantía de no-doble-reserva sigue siendo `UNIQUE(tenant_id, fecha)` de PostgreSQL. Dos "cambiar fecha" concurrentes hacia la misma `fecha_nueva` resultan en uno exitoso y uno con 409 (el perdedor de la restricción de unicidad) — nunca doble bloqueo. TDD de concurrencia cubrió al menos: dos cambios concurrentes a la misma `fecha_nueva` (solo uno gana), cambio concurrente con un alta que intenta bloquear la misma `fecha_nueva` (anti-doble-reserva D4), cambio que libera una fecha con cola (promoción FIFO exactamente-una-vez), y rollback total si falla el bloqueo de la nueva fecha. Fuente: `design.md §D-2.1`; `CLAUDE.md §Regla crítica: bloqueo atómico de fecha`.
