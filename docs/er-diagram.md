...
erDiagram
    ...
    COMUNICACION {
        ...
        enum subtipo "consulta_exploratoria | fecha_disponible | fecha_confirmada | cola_espera | cambio_fecha | NULL — nullable; NULL para E2-E4, E6-E7, E9-E10, manual y filas legadas"
        string asunto
        text cuerpo
...
3. **Aprobación y envío por el Gestor (borrador → enviada) — acción unificada US-023:** el Gestor pulsa "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`). En una única operación atómica: valida guardas (factura en `borrador`, `pdf_url` no nulo, sin COMUNICACION E3 `enviado` previa); envía E3 por `EnviarEmailPort` directo con la factura de señal como único adjunto (si el proveedor falla → `EmisionEnvioFallidoError` → rollback total); solo tras confirmar E3: `estado = 'enviada'`, `fecha_emision = now()`, INSERT COMUNICACION E3 `enviado`, AUDIT_LOG. Implementado en **US-023** (changes `documentos-enviar-factura-senal-e3` 6.4b + `condiciones-particulares-e3-us023`). **Nota (change `condiciones-idioma-e2-firma-banner`):** las condicions particulars **ya no se adjuntan ni se envían en E3** desde este change. El guard `CONDICIONES_NO_CONFIGURADAS` (409) y el PDF de condiciones se movieron a la confirmación del presupuesto (E2, UC-14): `GenerarPresupuestoUseCase.confirmar()` llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` pre-transacción; si devuelve `null` → 409 sin efectos; si tiene éxito → `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` se fijan en la misma transacción. E3 ya no crea ni reutiliza `DOCUMENTO(tipo='condiciones_particulares')` en su transacción.
...
- `enviada` → el Gestor ejecutó "Enviar factura 40%" (US-023); `fecha_emision` fijada; E3 confirmado al cliente (solo factura de señal; `RESERVA.cond_part_enviadas_fecha` se fija al confirmar el presupuesto en E2/UC-14, no al enviar E3).
...
1. **Guardas de precondición** (autoritativas, antes de mutar): `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas con E2 al confirmar presupuesto — si es nulo: 409 `CONDICIONES_NO_ENVIADAS`); `RESERVA.estado ∈ {reserva_confirmada, evento_en_curso, post_evento}` (si es terminal u otro: 422); fichero presente con `mimeType ∈ {image/jpeg, image/png, application/pdf}` y tamaño ≤ 10 MB (si no: 422). En cualquier rechazo: cero efectos en BD.
...
**Integración con E2 (change `condiciones-idioma-e2-firma-banner`):** desde este change, las condicions particulars se adjuntan en E2 al confirmar el presupuesto (`POST /reservas/{id}/presupuesto`, `GenerarPresupuestoUseCase.confirmar()`). El guard es ahora **duro en E2**: si `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` devuelve `null` (tenant sin condicions configuradas) → la confirmación aborta con 409 `CONDICIONES_NO_CONFIGURADAS` antes de abrir la transacción, sin ningún efecto sobre PRESUPUESTO, RESERVA ni FECHA_BLOQUEADA. `RESERVA.cond_part_enviadas_fecha` y `RESERVA.cond_part_firmadas = false` se fijan en la transacción de UC-14. `GenerarPdfCondicionesPort.generar()` requiere el parámetro `idioma: 'es' | 'ca'`; la clave de storage es `condiciones/{tenantId}-{idioma}.pdf`. Anteriormente (6.4a), E2 era post-commit fire-and-forget y omitía el adjunto sin error si no había condicions configuradas.

**Sin cambio de contrato OpenAPI para E2:** E2 es una operación post-commit interna. El envío E3 (`POST /reservas/{id}/facturas/senal/enviar`, operationId `enviarFacturaSenal`) fue implementado en 6.4b y completado en `condiciones-particulares-e3-us023`: acción manual del Gestor que aprueba+envía la factura de señal. **Desde el change `condiciones-idioma-e2-firma-banner`:** E3 envía únicamente la factura de señal; el PDF de condicions particulars ya no se adjunta ni se guarda como `DOCUMENTO` en la transacción E3. El reenvío manual se expone en `POST /reservas/{id}/facturas/senal/reenviar` (operationId `reenviarE3`): reutiliza la factura existente sin duplicar. Ver §3.13 (ciclo de vida de la factura de señal) y el diccionario de campos `cond_part_*` en §3.9.
...
| 6.4b | Envío E3 — `POST /reservas/{id}/facturas/senal/enviar` (US-023): acción manual del Gestor, atómica, espejo de E4; plantilla E3 activa. **Change `condiciones-idioma-e2-firma-banner`:** condicions particulars movidas de E3 a E2 (confirmar presupuesto); E3 envía solo la factura de señal. | Implementada |
