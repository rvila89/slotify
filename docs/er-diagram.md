...
| importe_total | DECIMAL(10,2) | Total congelado al confirmar el pago de la señal (UC-17/US-021). El sistema toma `PRESUPUESTO.total` del presupuesto vigente (`MAX(version)` con `estado='enviado'`), valida `> 0` y lo escribe en este campo dentro de la transacción atómica. Permanece `NULL` hasta esa transición. |
...
| cond_part_firmadas | BOOLEAN | Si las condiciones particulares están firmadas. **Desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** fijado a `false` al enviar E3 (US-023) **solo si** el adjunto de condicions particulars fue generado con éxito (adjunto no degradado); si el tenant no tiene condicions configuradas el campo no se modifica. A `true` al registrar la copia firmada (US-024). Governa el párrafo recordatorio de E4: si `false`, la liquidación incluye un párrafo recordatorio al cliente. Anteriormente (`condiciones-idioma-e2-firma-banner`) se fijaba a `false` al confirmar el presupuesto (E2/UC-14). Expuesto en el wire como `condPartFirmadas`. |
| cond_part_enviadas_fecha | TIMESTAMP | Fecha en que las condicions particulars fueron enviadas al cliente. **Desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** fijado al enviar E3 (US-023, `POST /reservas/{id}/facturas/senal/enviar`) **solo si** el PDF de condicions fue adjuntado (adjunto no degradado); si el tenant no tiene condicions configuradas permanece `NULL`. Se actualiza en cada reenvío manual de E3 que adjunte condicions. Precondición de US-024: si es nulo, el registro de firma se rechaza con 409 `CONDICIONES_NO_ENVIADAS`. Anteriormente (`condiciones-idioma-e2-firma-banner`) se fijaba al confirmar el presupuesto (E2/UC-14). |
...
**Transición pre_reserva → reserva_confirmada (US-021 / UC-17 — sin migración):** el Gestor sube el justificante del pago de la señal y confirma. La guarda de origen declarativa exige `estado = 'pre_reserva'`; cualquier otro estado produce 422 sin efectos. El fichero justificante debe tener `mime_type ∈ {image/jpeg, image/png, application/pdf}` y tamaño ≤ 10 MB (validación autoritativa en servidor). En una única transacción all-or-nothing serializada por `SELECT … FOR UPDATE` sobre la fila de `FECHA_BLOQUEADA` (reutilizando `bloquearFecha(fase='reserva_confirmada')` de US-040): (0) el sistema lee el presupuesto vigente de la reserva (`SELECT … WHERE reserva_id = X AND estado = 'enviado' ORDER BY version DESC LIMIT 1`) y valida `presupuesto.total > 0`; si no existe presupuesto vigente o `total <= 0`, aborta con 422 `IMPORTE_TOTAL_INVALIDO` sin ningún efecto; (1) INSERT de DOCUMENTO con `tipo = 'justificante_pago'`, `reserva_id`, `tenant_id`, `url` y `mime_type`; (2) UPDATE de RESERVA (`estado = 'reserva_confirmada'`, `ttl_expiracion = NULL`, `importe_total = presupuesto.total`, `importe_senal = round(importe_total × pct_senal / 100, 2)`, `importe_liquidacion = importe_total − importe_senal`, `pre_evento_status = 'pendiente'`, `liquidacion_status = 'pendiente'`, `fianza_status = 'pendiente'`); (3) UPDATE de PRESUPUESTO (`estado = 'aceptado'`) sobre el presupuesto vigente leído en el paso (0) — este UPDATE es el productor del estado `aceptado` en el PRESUPUESTO; (4) UPDATE de `FECHA_BLOQUEADA` a `tipo_bloqueo = 'firme'`, `ttl_expiracion = NULL` sin alterar `reserva_id` — upgrade de la fila existente, nunca DELETE+INSERT (ver §3.6); (5) INSERT idempotente de FICHA_OPERATIVA (todos los campos de contenido a NULL, `ficha_cerrada = false`; si ya existe con ese `reserva_id`, no se duplica y la transición continúa); (6) INSERT en AUDIT_LOG (`accion = 'transicion'`, `datos_anteriores.estado = 'pre_reserva'`, `datos_nuevos.estado = 'reserva_confirmada'`). Post-commit: se presenta la factura de señal en borrador al Gestor (disparo de UC-18 / US-022); no se genera condiciones particulares ni se envía E3 en US-021 (E3 se dispara solo tras la aprobación de la factura en US-022 y la generación de condiciones en US-023). El `pct_senal` se lee de `TENANT_SETTINGS` en el momento de confirmar, nunca hardcodeado. La liquidación se calcula como complemento (resta) para evitar desajuste de céntimos. Concurrencia: doble clic sobre la misma RESERVA → la segunda transacción observa `reserva_confirmada` bajo lock y devuelve "La reserva ya ha sido confirmada" (409); confirmar sobre fecha ya firme de otra RESERVA → `P2002` en `UNIQUE(tenant_id, fecha)` antes de mutar la segunda RESERVA → "Fecha no disponible" (409). Sin migración estructural: todos los campos (`reserva_confirmada` en el enum, `importe_total`, `importe_senal`, `importe_liquidacion`, enums de sub-procesos, `FICHA_OPERATIVA`, `DOCUMENTO.tipo = 'justificante_pago'`) ya estaban en el modelo. Fuente: US-021, UC-17; `design.md §D-1..D-8`; `er-diagram.md §3.6 mapa canónico fase reserva_confirmada`.
...
3. **Aprobación y envío por el Gestor (borrador → enviada) — acción unificada US-023:** el Gestor pulsa "Enviar factura 40%" (`POST /reservas/{id}/facturas/senal/enviar`). Antes de la transacción, el sistema intenta generar el PDF de condicions particulars vía `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` (**degradable**, `.catch(() => null)`): si el tenant no tiene condicions configuradas o el render falla, la URL es `null` y el adjunto se omite sin ningún error. En una única operación atómica: valida guardas (factura en `borrador`, `pdf_url` no nulo, sin COMUNICACION E3 `enviado` previa); envía E3 por `EnviarEmailPort` directo con la factura de señal y, **si la URL de condicions no es null**, el PDF de condicions particulars adjunto (si el proveedor falla → `EmisionEnvioFallidoError` → rollback total); solo tras confirmar E3: `estado = 'enviada'`, `fecha_emision = now()`, INSERT COMUNICACION E3 `enviado`, AUDIT_LOG; **si se adjuntaron condicions**: `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` dentro de la misma transacción (`fijarCondicionesEnviadas`). Implementado en **US-023** (change `condiciones-particulares-senal-y-recordatorio-liquidacion`). **Nota:** el guard duro `CONDICIONES_NO_CONFIGURADAS` (409) y la clase `CondicionesNoConfiguradasError` han sido eliminados: la operación es ahora siempre degradable. E2 (presupuesto) ya no invoca `GenerarPdfCondicionesPort` ni adjunta condicions particulars.
...
- `enviada` → el Gestor ejecutó "Enviar factura 40%" (US-023); `fecha_emision` fijada; E3 confirmado al cliente con la factura de señal y, si el tenant tiene condicions configuradas, el PDF de condicions particulars en blanco; `RESERVA.cond_part_enviadas_fecha` se fija en esta transición **solo si** se adjuntaron condicions (change `condiciones-particulares-senal-y-recordatorio-liquidacion`).
...
- Solicitud de datos fiscales al cliente (`POST /reservas/{id}/comunicaciones/solicitar-datos-presupuesto`, change `solicitud-datos-presupuesto-borrador`): `subtipo = 'solicitud_datos'` — borrador bilingüe (es/ca) que solicita nombre/apellidos, DNI y dirección/población; idempotente por la terna del índice UNIQUE parcial.
...
1. **Guardas de precondición** (autoritativas, antes de mutar): `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas con E3 al enviar la factura de señal, US-023, cuando el adjunto fue efectivo — si es nulo: 409 `CONDICIONES_NO_ENVIADAS`); `RESERVA.estado ∈ {reserva_confirmada, evento_en_curso, post_evento}` (si es terminal u otro: 422); fichero presente con `mimeType ∈ {image/jpeg, image/png, application/pdf}` y tamaño ≤ 10 MB (si no: 422). En cualquier rechazo: cero efectos en BD.
...
La rebanada 6.4a añade la generación del PDF de "Condicions particulars". El adjunto se movió a E3 (factura de señal) de forma degradable en el change `condiciones-particulares-senal-y-recordatorio-liquidacion`.
...
**Integración con E3 (change `condiciones-particulares-senal-y-recordatorio-liquidacion`):** las condicions particulars se adjuntan ahora en **E3** (factura de señal) de forma **degradable**: `EnviarFacturaSenalUseCase` llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` antes de la transacción (`.catch(() => null)`); si el tenant no tiene condicions configuradas o el render falla, la URL es `null`, el adjunto se omite y la señal se envía igualmente **sin ningún 409**. Cuando la URL no es `null`, el adjunto se añade a E3 y dentro de la transacción se fijan `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` (`fijarCondicionesEnviadas`). El reenvío manual E3 regenera el PDF en blanco vía `GenerarPdfCondicionesPort` (no busca un DOCUMENTO persistido). E2 ya no invoca `GenerarPdfCondicionesPort` ni adjunta condicions. La guarda dura `CONDICIONES_NO_CONFIGURADAS` (409) y la clase `CondicionesNoConfiguradasError` han sido eliminadas. `GenerarPdfCondicionesPort.generar()` requiere `idioma: 'es' | 'ca'`; la clave de storage es `condiciones/{tenantId}-{idioma}.pdf`. Anteriormente (`condiciones-idioma-e2-firma-banner`), las condicions se adjuntaban en E2 con guard duro; antes de ese change (6.4a) se adjuntaban en E3 de forma degradable.

**Histórico de integración:**
- 6.4a: E3 adjunta condicions de forma fire-and-forget post-commit (degradable).
- `condiciones-idioma-e2-firma-banner`: condicions movidas a E2 con guard duro 409.
- `condiciones-particulares-senal-y-recordatorio-liquidacion`: condicions devueltas a E3, degradable, sin guard duro. E4 incluye párrafo recordatorio cuando `cond_part_firmadas = false`.

**Sin cambio de contrato OpenAPI para E2:** E2 es una operación post-commit interna. El envío E3 (`POST /reservas/{id}/facturas/senal/enviar`, operationId `enviarFacturaSenal`) fue implementado en 6.4b: acción manual del Gestor que aprueba+envía la factura de señal. El reenvío manual se expone en `POST /reservas/{id}/facturas/senal/reenviar` (operationId `reenviarE3`): reutiliza la factura existente sin duplicar y regenera el PDF de condicions vía `GenerarPdfCondicionesPort` (degradable). Ver §3.13 (ciclo de vida de la factura de señal) y el diccionario de campos `cond_part_*` en §3.9.
...
| 6.4a | Condicions particulars: campo `condiciones` en `PlantillaDocumentoTenant`, plantilla react-pdf. **Historial adjunto:** 6.4a: E3 degradable → `condiciones-idioma-e2-firma-banner`: E2 con guard duro → `condiciones-particulares-senal-y-recordatorio-liquidacion`: E3 degradable sin guard duro (estado actual). | Implementada |
| 6.4b | Envío E3 — `POST /reservas/{id}/facturas/senal/enviar` (US-023): acción manual del Gestor, atómica, espejo de E4; plantilla E3 activa. **Change `condiciones-particulares-senal-y-recordatorio-liquidacion`:** E3 adjunta condicions particulars de forma degradable (`.catch(() => null)`); fija `cond_part_enviadas_fecha`/`cond_part_firmadas` solo si adjuntó; E2 adjunta solo el presupuesto; E4 incluye párrafo recordatorio condicional. | Implementada |
...
| US-023 | Envío E3 atómico con condicions degradables; reenvío E3 vía `POST /reservas/{id}/facturas/senal/reenviar` — `reenviarE3` regenera PDF en blanco via `GenerarPdfCondicionesPort`; E4 con párrafo recordatorio condicional (change `condiciones-particulares-senal-y-recordatorio-liquidacion`) | Implementada |
...
*Documento generado el 24/05/2026 como parte del modelado de datos del TFM de Slotify. Versión 5.3 (21/07/2026): refleja change `condiciones-idioma-e2-firma-banner` — actualiza §3.9 diccionario de campos `cond_part_firmadas` y `cond_part_enviadas_fecha` (ambos se fijan ahora al confirmar el presupuesto en E2/UC-14, no al enviar E3); actualiza §3.3 campo `condiciones` de `PLANTILLA_DOCUMENTO_TENANT` (guard duro pasa a estar en E2, no en E3); actualiza §3.13 paso 3 ciclo de vida de la factura de señal (E3 ya no adjunta condicions particulars; guard `CONDICIONES_NO_CONFIGURADAS` y fijado de `cond_part_*` se mueven a UC-14); actualiza §5.5 guardas del segundo flujo de UC-19 (`cond_part_enviadas_fecha` no nulo significa presupuesto confirmado vía E2, no E3 enviado); actualiza §5.10 integración E2/E3 (E2 pasa a ser guard duro; E3 solo envía factura de señal); actualiza roadmap épico #6 fila 6.4b. Sin cambio de modelo de datos (cero migraciones; los campos `cond_part_*` ya existían).*
...
*Documento generado el 24/05/2026 como parte del modelado de datos del TFM de Slotify. Versión 4.13 (21/07/2026): refleja change `fix-importe-total-confirmar-senal` — corrige y completa la documentación del campo `RESERVA.importe_total` en §3.6 (se congela en UC-17/US-021 tomando `PRESUPUESTO.total` del presupuesto vigente `MAX(version)` con `estado='enviado'`, validando `> 0` dentro de la transacción atómica; permanece NULL hasta esa transición; elimina referencia implícita a US-014 como momento de congelación); amplía la nota de la transición `pre_reserva → reserva_confirmada` en §3.6 (paso 0: lectura del presupuesto vigente y validación `> 0`; paso 2: escritura de `importe_total = presupuesto.total`; paso 3 nuevo: UPDATE del presupuesto vigente a `estado = 'aceptado'`; la guarda de origen ya no menciona `importe_total > 0` como precondición aislada, sino que la validación se hace dentro de la tx con el presupuesto real); corrige el bullet `aceptado` del mapa de estados de PRESUPUESTO en §3.12 (productor es UC-17/US-021 dentro de la transacción de confirmación, no un aceptado explícito del cliente). Sin migración de BD, sin cambio de contrato OpenAPI.*

*Documento generado el 24/05/2026 como parte del modelado de datos del TFM de Slotify. Versión 4.12 (20/07/2026): refleja change `historial-completo-comunicaciones` — añade atributo `subtipo` (enum `SubtipoEmail` nullable) al bloque COMUNICACION del diagrama Mermaid; añade fila `subtipo` al diccionario §3.17 COMUNICACION; actualiza la nota de idempotencia a la terna `(reserva_id, codigo_email, subtipo)` con `NULLS NOT DISTINCT` y predicado `estado='enviado'`; actualiza las notas de borrador E1 en la transición `2a→2b/2d` (añade `subtipo`); añade nota de borrador E1 en cambio de fecha de `2b` (nuevo comportamiento: `subtipo='cambio_fecha'`); actualiza la cobertura E1–E8 para describir el subtipo por evento; actualiza la entrada del índice UNIQUE parcial de COMUNICACION en §4.1 a la nueva clave de terna con `NULLS NOT DISTINCT` y `estado='enviado'`.*

*Documento generado el 24/05/2026 como parte del modelado de datos del TFM de Slotify. Versión 4.11 (20/07/2026): refleja change `presupuesto-edicion-reenvio-email-real` — actualiza §3.12 PRESUPUESTO flujo de edición UC-15 (paso 3 post-commit: `despacharReenvio` como fuente única de `COMUNICACION`, persistencia de `pdf_url`, variable `esEdicion`; paso 4 reenvío: uso de `pdf_url` vigente, `despacharReenvio` con `esEdicion=false`); actualiza §3.17 COMUNICACION cobertura E2 (precisa camino `despachar` en UC-14 vs `despacharReenvio` en UC-15; documenta la corrección del bug de envío real y la fuente única post-commit). Sin migración de BD ni cambios de contrato OpenAPI.*
