...
| **Postcondiciones** | - **Fecha libre:** consulta pasa a `2.b`; `fecha_evento` y `ttl_expiracion = now() + ttl_consulta_dias` escritos; fila insertada en `FECHA_BLOQUEADA` (`tipo_bloqueo='blando'`); `AUDIT_LOG` con `accion='transicion'`; borrador E1 "disponible" (plantilla dinámica bilingüe) creado en `estado='borrador'` dentro de la misma transacción, pendiente de revisión/envío manual por el gestor (flujo US-046); **sin auto-envío**<br>- **Fecha bloqueada por `2.b` + gestor acepta cola:** consulta pasa a `2.d`; `posicion_cola = MAX+1`; `consulta_bloqueante_id` apunta a la bloqueante; no se crea `FECHA_BLOQUEADA`; borrador E1 "cola" (plantilla dinámica bilingüe) creado en `estado='borrador'` dentro de la misma transacción, pendiente de revisión/envío manual (flujo US-046); **sin auto-envío**<br>- **Fecha bloqueada por `2.b` + gestor rechaza:** consulta permanece en `2.a` sin cambios (409 `colaDisponible:true`)<br>- **Fecha bloqueada por `2.c/2.v/pre_reserva/confirmada+`:** consulta permanece en `2.a` sin cambios (409 `colaDisponible:false`) |
| **Prioridad** | Alta |
| **Frecuencia** | Alta |
| **US** | US-005 |
| **Endpoint** | `POST /reservas/{id}/fecha` — body `{ fechaEvento: "YYYY-MM-DD", aceptarCola?: boolean }` |
| **Entidades afectadas** | RESERVA (UPDATE), FECHA_BLOQUEADA (INSERT en `2.b`), COMUNICACION (INSERT E1 borrador en `2.b` y `2.d`), AUDIT_LOG — sin migración de columnas |

> **Nota de trazabilidad US-051 / change `cambiar-fecha-consulta-en-cola` — cambiar una fecha:** este UC-04 cubre exclusivamente la asignación de fecha a una RESERVA en `2.a` (sin fecha previa). Para el caso en que la RESERVA **ya tiene** una fecha bloqueada (`sub_estado ∈ {2b, 2c, 2v}`) **o está en cola de espera** (`sub_estado = 2d`), existe una operación atómica dedicada — `POST /reservas/{id}/cambiar-fecha` — con semántica diferente según el origen: (a) desde `2b`/`2c`/`2v` (posee bloqueo propio) libera la fecha antigua y bloquea la nueva, con posible promoción FIFO de cola si había espera en la fecha antigua; (b) desde `2d` (no posee bloqueo propio) inserta un bloqueo nuevo de la fecha libre, transiciona `2d → 2b`, saca la RESERVA de la cola con reordenación y crea un borrador E1 — sin promover ninguna cola ni tocar la bloqueante antigua; si la fecha nueva está ocupada, en ambas ramas el sistema responde 409 terminal con rollback total. Cualquier otro origen produce 422 sin efectos. Ver `er-diagram.md §3.7` para la especificación técnica completa.
...
8. Dentro de la misma transacción: INSERT de `COMUNICACION E1` con `estado='borrador'`, plantilla "disponible" (bilingüe; `ca` → catalán, resto → castellano), asunto/cuerpo dinámicos con nombre, fecha del evento, personas y horas del evento (`___` si faltan), `fecha_envio=null`; **sin envío** (queda para revisión/envío manual del gestor, flujo US-046)
...
- **FA-01** (fecha bloqueada por `2.b` — flujo interactivo de cola):
  ...
  - Segunda llamada con `aceptarCola: true`: la RESERVA pasa a `2.d`; `posicion_cola = MAX(posicion_cola de esa fecha)+1` (serializado por `SELECT … FOR UPDATE` sobre la fila bloqueante); `consulta_bloqueante_id` y `fecha_evento` escritos; sin nueva `FECHA_BLOQUEADA`; INSERT de `COMUNICACION E1` con `estado='borrador'`, plantilla "cola" (bilingüe), sin auto-envío (flujo US-046). Responde `200` con `subEstado='2d'`, `posicionCola`, `consultaBloqueanteId`
...
flowchart TD
    ...
    F -->|Libre| G[Tx: UPDATE RESERVA 2b + INSERT FECHA_BLOQUEADA blando + INSERT COMUNICACION E1 borrador disponible + AUDIT_LOG transicion]
    G --> I[200 — subEstado=2b + ttlExpiracion]
    F -->|Bloqueada por 2b| J{aceptarCola?}
    J -->|false / ausente| K[409 — colaDisponible:true]
    J -->|true| L[Tx: UPDATE RESERVA 2d + posicion_cola + consulta_bloqueante_id + INSERT COMUNICACION E1 borrador cola + AUDIT_LOG]
...
| **Actor Principal** | Gestor |
| **Actores Secundarios** | Sistema |
| **Descripción** | El Gestor genera un presupuesto formal cuando el cliente ha confirmado todos los datos necesarios, activando la transición al estado `pre_reserva`. El flujo se divide en dos fases: **preview** (borrador editable, sin persistencia) y **confirmación** (transacción única que coordina PRESUPUESTO + RESERVA + FECHA_BLOQUEADA + cola + AUDIT_LOG). Implementado en **US-014** (change `us-014-generar-presupuesto-activar-prereserva`). Extendido en **6.2** (`documentos-presupuesto-sin-iva-doble-numeracion`): `metodoPago` obligatorio en preview y confirmación; el régimen fiscal determina el cálculo y la variante del PDF. Modificado en **change `condiciones-particulares-senal-y-recordatorio-liquidacion`**: la fase de confirmación ya **no** invoca `GenerarPdfCondicionesPort` ni adjunta condicions particulars al email E2; E2 adjunta únicamente el PDF del presupuesto. Las condicions particulars pasan a adjuntarse en E3 (factura de señal) de forma degradable. Se eliminan la guarda dura `CONDICIONES_NO_CONFIGURADAS` (409) y la fijación de `cond_part_enviadas_fecha`/`cond_part_firmadas` dentro de `confirmar()`. |
| **Precondiciones** | - RESERVA en `estado = 'consulta'`, `sub_estado ∈ {'2a','2b','2c','2v'}` (guarda declarativa `ORIGENES_TRANSICION_ACTIVAR_PRERESERVA`; `2d`, terminales y `pre_reserva`/posteriores se rechazan sin efectos)<br>- **No existe** PRESUPUESTO en `estado = 'enviado'` o `'aceptado'` para esa RESERVA (si existe, remitir a UC-15)<br>- Datos de reserva completos: `fecha_evento` (futura), `duracion_horas ∈ {4,8,12}`, `num_adultos_ninos_mayores4 ≥ 1`, `tipo_evento ∈ {boda,corporativo,privado,otro}`<br>- Datos fiscales del CLIENTE no nulos ni vacíos: `dni_nif`, `direccion`, `codigo_postal`, `poblacion`, `provincia` (si faltan, error enumera campos faltantes vía `camposFaltantes` en el cuerpo del 422, propagado por `HttpExceptionFilter`)<br>- `metodoPago ∈ {transferencia, efectivo}` presente en el request (obligatorio desde 6.2; ausencia → 400)<br>- Gestor autenticado con rol gestor sobre el tenant<br>- **[US-051] Gating de completitud en UI (no nueva precondición de servidor):** desde US-051, la ficha deshabilita el botón "Generar presupuesto" en UI hasta que estén presentes `fechaEvento` + `numAdultosNinosMayores4` + `duracionHoras` + `horario`; indica qué campo falta y sugiere "Editar consulta". El servidor sigue revalidando su propio conjunto de campos obligatorios (sin `horario`); el gating de `horario` es exclusivamente de UI y mejora la experiencia evitando el flujo de error 422 en cascada. |
| **Postcondiciones** | - PRESUPUESTO creado con `version = 1`, `tarifa_congelada = true`, `estado = 'enviado'`, desglose fiscal congelado según régimen (`iva_porcentaje = 21` en CON IVA; `iva_porcentaje = 0` en SIN IVA), `metodoPago` y `regimenIva` persistidos<br>- RESERVA pasa a `estado = 'pre_reserva'`, `ttl_expiracion = now() + TENANT_SETTINGS.ttl_prereserva_dias` (7 días por defecto, nunca hardcodeado)<br>- `RESERVA.cond_part_enviadas_fecha` y `cond_part_firmadas` **no se modifican** en esta transición (se fijan en E3 cuando se adjuntan condicions, change `condiciones-particulares-senal-y-recordatorio-liquidacion`)<br>- FECHA_BLOQUEADA: si origen `2b`/`2c`/`2v` → UPDATE del `ttl_expiracion`; si origen `2a` → INSERT nueva fila; `tipo_bloqueo = 'blando'` en ambos casos<br>- Cola vaciada: todas las RESERVA con `consulta_bloqueante_id = id` y `sub_estado = '2d'` pasan a `'2y'`, `posicion_cola = NULL`, `consulta_bloqueante_id = NULL` (mecánica A16)<br>- `AUDIT_LOG accion='transicion'` para la RESERVA principal y para cada RESERVA descartada de la cola<br>- Post-commit: PDF generado en la variante del régimen (CON IVA o SIN IVA, react-pdf), `PRESUPUESTO.pdf_url` actualizado; E2 disparado vía motor US-045 con el PDF del presupuesto **únicamente** (fallo del proveedor no revierte la pre-reserva; queda trazado en `COMUNICACION`) |
| **Prioridad** | Crítica |
| **Frecuencia** | Alta |
| **US** | US-014 |
...
11. En una **única transacción all-or-nothing** serializada por `SELECT … FOR UPDATE` sobre la fila de `FECHA_BLOQUEADA`:
    ...
    - UPDATE RESERVA: `estado = 'pre_reserva'`, `sub_estado = NULL`, `ttl_expiracion = now() + ttl_prereserva_dias` (`cond_part_enviadas_fecha` y `cond_part_firmadas` **no se modifican** aquí — change `condiciones-particulares-senal-y-recordatorio-liquidacion`)
...
12. Post-commit (no bloqueante): generación del PDF en la variante del régimen (react-pdf) + UPDATE de `PRESUPUESTO.pdf_url`; disparo de E2 vía motor US-045 con el PDF del presupuesto **únicamente** (sin condicions particulars — change `condiciones-particulares-senal-y-recordatorio-liquidacion`). Un fallo en la generación del PDF o en el envío del email no revierte la pre-reserva
...
**Flujos Alternativos:**
- **FA-01** (datos fiscales incompletos): el sistema responde `422` con `camposFaltantes: ['dni_nif', ...]` enumerando los campos fiscales que faltan; sin efectos sobre PRESUPUESTO, RESERVA ni FECHA_BLOQUEADA. El `HttpExceptionFilter` propaga `camposFaltantes` en el body del error. La UI resalta/enfoca los campos faltantes y permite completarlos inline mediante `PATCH /reservas/{id}/datos-fiscales` (FA-08) antes de reintentar la generación del presupuesto. Como alternativa, el gestor puede solicitar al cliente que los aporte directamente mediante `POST /reservas/{id}/comunicaciones/solicitar-datos-presupuesto` (change `solicitud-datos-presupuesto-borrador`), que genera un borrador E1 con `subtipo = 'solicitud_datos'` en el idioma de la reserva; el borrador se revisa y envía vía UC-36.
...
flowchart TD
    ...
    T --> U[Post-commit: PDF presupuesto → pdf_url UPDATE + E2 motor US-045 solo presupuesto]
```
...
8. Post-commit: se presenta al Gestor la factura de señal en borrador para revisión (disparo de UC-18 / US-022); el Gestor la aprobará y enviará vía E3 en UC-19 / US-023 (la factura de señal y, si el tenant tiene condicions configuradas, el PDF de condicions particulars en blanco de forma degradable — change `condiciones-particulares-senal-y-recordatorio-liquidacion`)
...
11. La factura queda lista para que el Gestor ejecute el envío E3 mediante UC-19 / US-023 (`POST /reservas/{id}/facturas/senal/enviar`), que aprueba+envía en una sola operación atómica (la factura de señal y, si el tenant tiene condicions configuradas, el PDF de condicions particulars en blanco de forma degradable — change `condiciones-particulares-senal-y-recordatorio-liquidacion`)
...
| **Actor Principal** | Gestor |
| **Actores Secundarios** | Sistema, Cliente |
| **Descripción** | El gestor gestiona el ciclo de vida del documento de condicions particulars: aprobación y envío manual al cliente (primer flujo, US-023) y registro de la copia firmada (segundo flujo, US-024). **Desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`**, el primer envío de condicions particulars ocurre en **E3** (factura de señal), no en E2: `EnviarFacturaSenalUseCase` llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` de forma **degradable** (`.catch(() => null)`) antes de la transacción; si el tenant no tiene condicions configuradas o el render falla, el adjunto se omite y la señal se envía igualmente **sin ningún 409**; si el PDF se genera, se añade al email E3 y dentro de la transacción se fijan `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` (`fijarCondicionesEnviadas`). El reenvío manual (E3) regenera el PDF en blanco vía `GenerarPdfCondicionesPort` (no busca un DOCUMENTO persistido). La factura de liquidación (E4) incluye un **párrafo recordatorio** al cliente cuando `RESERVA.cond_part_firmadas = false`. El registro de la firma se realiza subiendo la copia firmada mediante `POST /reservas/{id}/condiciones-firmadas`; no transiciona estado. Implementado en **US-023** y **US-024**. |
| **Precondiciones** | Primer flujo (US-023): reserva en `reserva_confirmada`; factura de señal (`tipo='senal'`) en `estado='borrador'` con `pdf_url` no nulo; Gestor autenticado con rol gestor. (El adjunto de condicions es degradable: la operación no se bloquea si el tenant no tiene condicions configuradas.) Segundo flujo (US-024): `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas en E3); `RESERVA.estado ∈ {reserva_confirmada, evento_en_curso, post_evento}`; Gestor autenticado con rol gestor. |
| **Postcondiciones** | Primer flujo: `FACTURA(senal).estado = 'enviada'`; `COMUNICACION E3` con `estado='enviado'`; `AUDIT_LOG`; **si** se adjuntaron condicions: `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` (change `condiciones-particulares-senal-y-recordatorio-liquidacion`); si no, ambos campos permanecen sin cambio. Segundo flujo: nueva fila `DOCUMENTO(tipo='condiciones_particulares')` firmada; `RESERVA.cond_part_firmadas = true`; `RESERVA.cond_part_firmadas_fecha = now()`; `AUDIT_LOG accion='actualizar'`; `RESERVA.estado` sin cambio. |
| **Prioridad** | Alta |
| **Frecuencia** | Alta |
...
| **Entidades afectadas** | Primer flujo: FACTURA (UPDATE estado borrador→enviada, fecha_emision), COMUNICACION (INSERT E3 enviado), AUDIT_LOG; RESERVA (UPDATE `cond_part_enviadas_fecha`, `cond_part_firmadas` **solo si** se adjuntaron condicions — change `condiciones-particulares-senal-y-recordatorio-liquidacion`). Segundo flujo: DOCUMENTO (INSERT tipo=condiciones_particulares, copia firmada — no idempotente), RESERVA (UPDATE cond_part_firmadas, cond_part_firmadas_fecha), AUDIT_LOG. Sin migración BD en ambos flujos. |
...
3. Antes de la transacción, el sistema intenta generar el PDF de condicions particulars vía `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` (**degradable**, `.catch(() => null)`): si el tenant no tiene condicions configuradas o el render falla, la URL es `null` y el adjunto se omite sin ningún error; si tiene éxito, la URL queda disponible para añadir al email
4. El sistema valida: existe FACTURA de tipo `senal` para la reserva (404 si no); la factura está en `borrador` (409 si `rechazada`); no existe COMUNICACION E3 `enviado` previa (409 `E3_YA_ENVIADO` si ya se envió); `pdf_url` no nulo (502 si nulo)
5. En una única unidad de trabajo atómica (`tx + RLS`): envía E3 por `EnviarEmailPort` directo con la factura de señal y, si la URL de condicions no es `null`, el PDF de condicions particulars adjunto. Si el proveedor de email falla → `EmisionEnvioFallidoError` (502), **rollback total** (nada se consolida)
6. Tras confirmar E3: transiciona la FACTURA `borrador → enviada`; `fecha_emision = now()`; crea COMUNICACION E3 `enviado`; registra AUDIT_LOG; **si se adjuntaron condicions**: actualiza `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` (`fijarCondicionesEnviadas`) dentro de la misma transacción
7. El sistema responde 200 con la factura emitida y `condPartEnviadasFecha` (el nuevo valor si se adjuntaron condicions, o el valor previo si no)

**Flujo Básico — reenvío manual de E3 (US-023):**
...
3. Antes del envío, el sistema regenera el PDF de condicions particulars en blanco vía `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` (**degradable**, `.catch(() => null)`): regenera siempre un PDF nuevo (no busca un DOCUMENTO persistido, que ya no existe desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`); si devuelve `null`, el reenvío adjunta solo la factura de señal
4. Reutiliza el PDF de la factura de señal ya emitida (no regenera ni duplica la FACTURA)
5. El envío es síncrono por `EnviarEmailPort` directo y ocurre **antes** de tocar la BD: si el proveedor falla, el reenvío aborta con error recuperable y **no crea** la COMUNICACION de reenvío ni modifica `cond_part_enviadas_fecha`
6. Tras confirmar: crea nueva COMUNICACION E3 `es_reenvio=true, estado='enviado'`; registra AUDIT_LOG. NO transiciona FACTURA ni reserva. **Si se adjuntaron condicions**: actualiza `RESERVA.cond_part_enviadas_fecha` al nuevo timestamp; si no, el campo permanece sin cambio
7. El sistema responde 200
...
3. El sistema valida (guardas de precondición, autoritativas, antes de mutar):
   - `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas con E3 al enviar la factura de señal, US-023). Si es nulo → 409 `CONDICIONES_NO_ENVIADAS` "Las condiciones particulares no han sido enviadas al cliente aún"; sin efectos
...
**Re-firma (no idempotente por diseño — N2):** si `cond_part_firmadas` ya es `true` (p. ej. subir una versión más legible), la operación no se rechaza. El sistema crea otra fila `DOCUMENTO` de `tipo = 'condiciones_particulares'` (nueva versión), actualiza `cond_part_firmadas_fecha` al nuevo timestamp, mantiene `cond_part_firmadas = true` y conserva todas las versiones anteriores de `DOCUMENTO` y de binarios. `AUDIT_LOG.datos_anteriores.cond_part_firmadas = true` en este caso. El PDF de condicions en blanco lo genera `EnviarFacturaSenalUseCase` en el envío E3 (US-023, change `condiciones-particulares-senal-y-recordatorio-liquidacion`); la copia firmada es versionable (una por registro de firma de US-024).
...
- **FA-04** (condiciones no configuradas — degradable, no bloqueante): desde el change `condiciones-particulares-senal-y-recordatorio-liquidacion`, si `GenerarPdfCondicionesPort.generar()` devuelve `null` (tenant sin condicions configuradas o error de render), el adjunto de condicions se omite y E3 se envía igualmente con solo la factura de señal; `cond_part_enviadas_fecha` no se modifica. No hay ningún 409.
...
- **FA-01** (condiciones no enviadas): `cond_part_enviadas_fecha` nulo → 409 `CONDICIONES_NO_ENVIADAS`; sin efectos en BD. La acción "Registrar condiciones firmadas" no está disponible hasta que se haya enviado la factura de señal (E3, US-023) **con** el adjunto de condicions (el campo solo se fija cuando el adjunto fue efectivo).
...
| **Actor Principal** | Gestor |
| **Actores Secundarios** | Sistema |
| **Descripción** | El gestor sube la documentación obligatoria durante el evento (foto DNI anverso, foto DNI reverso, cláusula de responsabilidad firmada) y consulta el checklist de completitud en tiempo real. Implementado en **US-033** (change `us-033-capturar-documentacion-evento`). |
| **Precondiciones** | - Reserva en `estado = evento_en_curso` (guarda de precondición declarativa `esEstadoQuePermiteDocumentacionEvento`; solo lectura de checklist también en `post_evento`)<br>- Gestor autenticado con rol gestor sobre el tenant |
| **Postcondiciones** | **Subida (POST):** fila `DOCUMENTO` creada con `tipo ∈ {dni_anverso, dni_reverso, clausula_responsabilidad}`, bytes persistidos en almacén durable (`documentos-evento/{tenantId}/{reservaId}/{tipo}/{uuid}.ext`); `AUDIT_LOG accion='crear'`, `entidad='DOCUMENTO'`; checklist devuelto en la misma respuesta (201). `RESERVA.estado` y los sub-procesos **permanecen intactos** (no es transición de la máquina de estados).<br>**Checklist (GET):** lectura derivada de las filas `DOCUMENTO` existentes — para cada uno de los 3 tipos, `completado = existe ≥ 1 DOCUMENTO de ese tipo` para la reserva; `documento` (opcional) es el más reciente. El estado de checklist **no se materializa** en la reserva; se calcula en cada lectura.|
| **Prioridad** | Alta |
| **Frecuencia** | Media |
| **US** | US-033 |
| **Endpoints** | `POST /reservas/{id}/documentos-evento` — multipart/form-data, campos `archivo` (binario) + `tipo` (`dni_anverso` \| `dni_reverso` \| `clausula_responsabilidad`) → 201 `SubirDocumentoEventoResponse` (documento + checklist); 422 (`ESTADO_NO_PERMITE_DOCUMENTACION`, `TIPO_DOCUMENTO_NO_PERMITIDO`, `ARCHIVO_REQUERIDO`, `FORMATO_NO_PERMITIDO`, `ARCHIVO_INVALIDO`, `TAMANO_EXCEDIDO`); 404<br>`GET /reservas/{id}/documentos-evento/checklist` → 200 `ChecklistDocumentacionEvento` (3 ítems; consultable también en `post_evento`) |
| **Entidades afectadas** | DOCUMENTO (INSERT), AUDIT_LOG — sin migración de esquema (`TipoDocumento`, tabla `DOCUMENTO` y `AlmacenDocumentosPort` ya existían) |

**Flujo Básico (subida):**
1. El gestor accede a la ficha de la reserva en `estado = evento_en_curso` (vista responsive mobile-first)
2. El gestor consulta el checklist: `GET /reservas/{id}/documentos-evento/checklist` — 3 ítems (`dni_anverso`, `dni_reverso`, `clausula_responsabilidad`), cada uno con `completado` y el documento más reciente si existe
3. El gestor selecciona un tipo, adjunta el archivo (imagen JPEG/PNG o PDF) y confirma
4. El frontend llama a `POST /reservas/{id}/documentos-evento` con `multipart/form-data` (campos `archivo` + `tipo`)
5. El sistema valida la guarda de precondición: `RESERVA.estado = evento_en_curso`
6. El sistema valida el archivo: `mimeType ∈ {image/jpeg, image/png, application/pdf}`, tamaño > 0 y ≤ 10 MB, archivo presente
7. El sistema valida que `tipo ∈ {dni_anverso, dni_reverso, clausula_responsabilidad}`
8. El sistema sube los bytes al almacén durable con clave `documentos-evento/{tenantId}/{reservaId}/{tipo}/{uuid}.{ext}`
9. El sistema crea la fila `DOCUMENTO` (`tipo`, `url`, `mimeType`, `nombreArchivo`, `tamanoBytes`, `reservaId`, `tenantId`) — **sin buscar antes** (no idempotente; el histórico de versiones se conserva, la tabla no impone unicidad por `(reserva_id, tipo)`)
10. El sistema registra `AUDIT_LOG accion='crear'`, `entidad='DOCUMENTO'` con el `usuario_id` del Gestor
11. El sistema calcula el checklist actualizado y devuelve **201** `SubirDocumentoEventoResponse` (DOCUMENTO creado + checklist)

**Flujos Alternativos:**
- **FA-01 (documentación incompleta al finalizar evento):** el checklist con ítems pendientes se expone como señal informativa no bloqueante. La documentación incompleta **no impide** la transición a `post_evento` (UC-25 / US-034); el checklist sigue siendo consultable en `post_evento` para subidas tardías. No hay ningún cron asociado a US-033.
- **FA-02 (estado incorrecto — guarda de precondición):** `RESERVA.estado ≠ evento_en_curso` al hacer POST → 422 `ESTADO_NO_PERMITE_DOCUMENTACION`; ningún efecto en BD ni en almacén.
- **FA-03 (archivo ausente):** campo `archivo` no enviado → 422 `ARCHIVO_REQUERIDO`; sin efectos.
- **FA-04 (formato no permitido):** `mimeType ∉ {image/jpeg, image/png, application/pdf}` → 422 `FORMATO_NO_PERMITIDO`; sin efectos.
- **FA-05 (archivo vacío o corrupto):** `tamanoBytes = 0` o lectura fallida → 422 `ARCHIVO_INVALIDO`; sin efectos.
- **FA-06 (tamaño excedido):** `tamanoBytes > 10 MB` → 422 `TAMANO_EXCEDIDO`; sin efectos.
- **FA-07 (tipo no permitido):** `tipo ∉ {dni_anverso, dni_reverso, clausula_responsabilidad}` → 422 `TIPO_DOCUMENTO_NO_PERMITIDO`; sin efectos.
- **FA-08 (re-subida del mismo tipo):** se admite sin error. Crea una nueva fila `DOCUMENTO` (nueva versión); `completado` permanece `true`; el documento de referencia del checklist pasa a ser el más reciente. Histórico preservado.

```mermaid
flowchart TD
    A[Gestor: subir documento del evento] --> B{RESERVA.estado = evento_en_curso?}
    B -->|No| C[422 ESTADO_NO_PERMITE_DOCUMENTACION — sin efectos]
    B -->|Sí| D{archivo presente?}
    D -->|No| E[422 ARCHIVO_REQUERIDO]
    D -->|Sí| F{mimeType permitido?}
    F -->|No| G[422 FORMATO_NO_PERMITIDO]
    F -->|Sí| H{tamanoBytes > 0 y ≤ 10 MB?}
    H -->|Vacío| I[422 ARCHIVO_INVALIDO]
    H -->|> 10 MB| J[422 TAMANO_EXCEDIDO]
    H -->|OK| K{tipo ∈ dni_anverso|dni_reverso|clausula_responsabilidad?}
    K -->|No| L[422 TIPO_DOCUMENTO_NO_PERMITIDO]
    K -->|Sí| M[Subir bytes al almacén durable — clave documentos-evento/tenant/reserva/tipo/uuid.ext]
    M --> N[INSERT DOCUMENTO — tipo, url, mimeType, nombreArchivo, tamanoBytes, reservaId, tenantId]
    N --> O[INSERT AUDIT_LOG accion=crear entidad=DOCUMENTO usuario_id del Gestor]
    O --> P[Calcular checklist — para cada tipo: completado = existe ≥ 1 DOCUMENTO, documento = el más reciente]
    P --> Q[201 SubirDocumentoEventoResponse — DOCUMENTO creado + checklist]
```

**Nota de alcance (US-033):**
- **Implementado:** subida multipart de los 3 tipos obligatorios (`dni_anverso`, `dni_reverso`, `clausula_responsabilidad`) solo en `evento_en_curso`, checklist derivado en tiempo real consultable también en `post_evento`, histórico no idempotente (una fila nueva por subida), almacén durable con clave versionada, validación autoritativa de mime/tamaño/tipo en servidor, `AUDIT_LOG accion='crear'`, RLS multi-tenant. `DocumentoRepositoryPort` generalizado: `tipo` pasa de literal `'condiciones_particulares'` a union `TipoDocumentoDominio`; método `listarPorReservaYTipos` añadido (aditivo, sin romper US-023).
- **Fuera de alcance (📐):** la advertencia de documentación pendiente al finalizar el evento es de UC-25 / US-034 (ya implementada); no hay cron ni barrido en US-033.
- **Módulo:** M7 (Slotify Brief). Capability OpenSpec: `documentacion-evento`. Automatización: A30.
- **US coordinadas:** US-031 (precondición del estado `evento_en_curso`); US-034 (consulta el checklist al finalizar, advertencia informativa no bloqueante); US-023 (idempotencia de condiciones particulares; convive con la no-idempotencia de US-033 en la misma tabla `DOCUMENTO`).
...
|----|---------|-----------|-----------------|
| E1 | (1) Lead entrante (UC-03) · (2) Transición de fecha `2.a → 2.b` (disponible) o `2.a → 2.d` (cola) (UC-04 / US-005) · (3) Solicitud de datos fiscales al cliente cuando están incompletos (`POST /reservas/{id}/comunicaciones/solicitar-datos-presupuesto`, change `solicitud-datos-presupuesto-borrador`) | (1) Respuesta inicial + tarifa estimada · (2) Borrador dinámico bilingüe ("disponible" o "cola"), sin auto-envío; revisión/envío manual del gestor (flujo US-046) · (3) Borrador bilingüe solicitando nombre/apellidos, DNI y dirección/población; `subtipo = 'solicitud_datos'`; revisión/envío manual del gestor (flujo US-046) | Activa |
...
| E3 | Acción manual del gestor: "Enviar factura 40%" — `POST /reservas/{id}/facturas/senal/enviar` (US-023, 6.4b; condicions particulars adjuntas de forma degradable en E3 desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`) | Factura 40% (señal) + PDF condicions particulars en blanco (degradable, omitido si el tenant no las tiene configuradas) | Activa |
| E4 | Acción manual del gestor: "Aprobar y enviar liquidación" — `POST /reservas/{id}/facturas/liquidacion/enviar` (US-028; párrafo recordatorio condicions firmadas pendientes cuando `cond_part_firmadas = false` desde change `condiciones-particulares-senal-y-recordatorio-liquidacion`) | Factura liquidación (párrafo recordatorio condicional) | Activa |
...
>
> **Historial de condicions particulars en el flujo de emails:**
> - Hasta change `condiciones-idioma-e2-firma-banner` (2026-07-21): E3 (señal) adjuntaba el PDF de condicions particulars con guarda dura.
> - Change `condiciones-idioma-e2-firma-banner`: condicions movidas a E2 (presupuesto) con guarda dura 409 `CONDICIONES_NO_CONFIGURADAS`.
> - **Change `condiciones-particulares-senal-y-recordatorio-liquidacion` (2026-07-24):** condicions devueltas a **E3** (señal) de forma **degradable** (`.catch(() => null)`): si el tenant no tiene condicions configuradas, el adjunto se omite y la señal se envía igual sin ningún 409. Se elimina la guarda dura y la clase `CondicionesNoConfiguradasError`. E2 adjunta únicamente el PDF del presupuesto. Cuando se adjuntan condicions en E3, se fijan `RESERVA.cond_part_enviadas_fecha = now()` y `RESERVA.cond_part_firmadas = false` dentro de la transacción. La factura de liquidación E4 incluye un párrafo recordatorio al cliente cuando `RESERVA.cond_part_firmadas = false`. El parámetro `idioma: 'es' | 'ca'` sigue siendo requerido en `GenerarPdfCondicionesPort.generar()` y la clave de almacenamiento del PDF sigue siendo `condiciones/{tenantId}-{idioma}.pdf`.
>
...
| UC-24 | Capturar Documentación | Gestor | Medio | Alta | Baja |
...
*Documento generado el 22/05/2026 como parte del análisis de requisitos del TFM de Slotify. Versión 2.4 (24/07/2026): refleja change `condiciones-particulares-senal-y-recordatorio-liquidacion` — UC-14 (Confirmar Presupuesto): elimina la guarda pre-transacción de condicions (`CONDICIONES_NO_CONFIGURADAS`, FA-09), retira `cond_part_enviadas_fecha`/`cond_part_firmadas` de las postcondiciones y de la transacción, E2 adjunta solo el presupuesto (sin condicions); UC-19 (Gestionar Condiciones Particulares): reescribe el primer flujo (E3): adjunto de condicions particulars degradable en E3 (`fijarCondicionesEnviadas` si URL no null), reenvío regenera PDF vía `GenerarPdfCondicionesPort` (sin buscar DOCUMENTO); actualiza precondiciones, postcondiciones y entidades afectadas; actualiza FA-01/FA-04 del primer flujo; actualiza FA-01 del segundo flujo (US-024); actualiza catálogo de emails (E3 con adjunto degradable, E4 con recordatorio condicional); añade nota histórica de condicions en §catálogo. Sin migración de BD.*

*Documento generado el 22/05/2026 como parte del análisis de requisitos del TFM de Slotify. Versión 2.3 (21/07/2026): refleja change `condiciones-idioma-e2-firma-banner` — actualiza UC-14 (Confirmar Presupuesto / Activar Pre-reserva): añade guarda pre-transacción `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` (FA-09, 409 `CONDICIONES_NO_CONFIGURADAS`), fijado de `RESERVA.cond_part_enviadas_fecha = now()` y `cond_part_firmadas = false` en la transacción, adjunto de condicions particulars en E2; actualiza diagrama Mermaid UC-14; actualiza UC-17 (paso 8) y UC-18 (paso 11) para aclarar que E3 solo lleva la factura de señal; reescribe UC-19 (primer flujo): E3 ya no adjunta condicions particulars, `FacturaSenalEnvioResponse` ya no tiene `condPartAdjuntada`, entidades afectadas E3 no tocan RESERVA, FA-04 refiere al guard en UC-14; actualiza la tabla de emails MVP (E3: "Factura 40% (señal)"; añade nota `condiciones-idioma-e2-firma-banner` en §Nota del catálogo). Sin migración de BD ni cambio de contrato OpenAPI gestionado por este doc.*
...
*Documento generado el 22/05/2026 como parte del análisis de requisitos del TFM de Slotify. Versión 2.0 (18/07/2026): refleja el change `email-transicion-fecha-borrador` (US-005 / UC-04): el E1 de la transición de fecha ya NO se auto-envía; se crea en `estado='borrador'` dentro de la misma transacción para revisión/envío manual del gestor (flujo US-046). La rama de cola (`2.a → 2.d`, gestor acepta cola) también genera un borrador E1 con plantilla "cola". Las plantillas son dinámicas y bilingües (`ca` → catalán, resto → castellano), con nombre del cliente, fecha del evento, personas (`num_invitados_final`) y horas (`duracion_horas`); placeholder `___` si faltan personas o horas. El caso no encolable (409 `colaDisponible:false`) sigue sin correo. Sin cambios de contrato OpenAPI/SDK, frontend ni migración de columnas. Actualiza postcondiciones, flujo básico paso 8, FA-01 segunda llamada y diagrama Mermaid de UC-04.*
*Documento generado el 22/05/2026 como parte del análisis de requisitos del TFM de Slotify. Versión 1.9 (18/07/2026): refleja US-051 — Ficha de consulta: ver detalles, editar y sanear acciones. UC-04: añade nota de trazabilidad sobre la operación atómica "cambiar fecha ya bloqueada" (`POST /reservas/{id}/cambiar-fecha`, operación nueva de US-051 para `sub_estado ∈ {2b,2c,2v}`) que complementa —sin reemplazar— el flujo `2a → 2b` de UC-04. UC-14: añade nota de gating de completitud de UI (US-051 §D-3): el botón "Generar presupuesto" de la ficha solo se habilita cuando están presentes `fechaEvento` + `numAdultosNinosMayores4` + `duracionHoras` + `horario`; no es una nueva precondición de servidor sino un control de UI que evita el 422 en cascada. Sin nuevos casos de uso ni migración de esquema (US-051 reutiliza `RESERVA` y `FECHA_BLOQUEADA` existentes; `horario` ya existía desde US-047). Las acciones de la ficha en sub-estados/estados terminales (`2x`/`2y`/`2z`/`reserva_cancelada`/`reserva_completada`) muestran solo el fallback "No hay acciones disponibles" (saneo de guardas de UI, sin impacto en la máquina de estados del servidor).*
