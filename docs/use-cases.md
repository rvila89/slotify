...
10b. **Guarda pre-transacción de condiciones** (change `condiciones-idioma-e2-firma-banner`): el sistema llama a `GenerarPdfCondicionesPort.generar({ tenantId, idioma: reserva.idioma })` antes de abrir la transacción; si devuelve `null` → 409 `CONDICIONES_NO_CONFIGURADAS` sin ningún efecto sobre PRESUPUESTO, RESERVA ni FECHA_BLOQUEADA; si lanza excepción transitoria → 502 sin efectos
11. En una **única transacción all-or-nothing** serializada por `SELECT … FOR UPDATE` sobre la fila de `FECHA_BLOQUEADA`:
    ...
    - UPDATE RESERVA: `estado = 'pre_reserva'`, `sub_estado = NULL`, `ttl_expiracion = now() + ttl_prereserva_dias`, `cond_part_enviadas_fecha = now()`, `cond_part_firmadas = false`
...
12. Post-commit (no bloqueante): generación del PDF en la variante del régimen (react-pdf) + UPDATE de `PRESUPUESTO.pdf_url`; disparo de E2 vía motor US-045 con el PDF del presupuesto y el PDF de condicions particulars adjuntos. Un fallo en la generación del PDF o en el envío del email no revierte la pre-reserva
...
- **FA-09** (condiciones particulares no configuradas — guarda pre-transacción, change `condiciones-idioma-e2-firma-banner`): `GenerarPdfCondicionesPort.generar({ tenantId, idioma })` devuelve `null` (tenant sin condiciones configuradas o sin secciones en `PlantillaDocumentoTenant.condiciones`) → **409 `CONDICIONES_NO_CONFIGURADAS`** "Configura las condiciones particulares del espacio antes de generar el presupuesto"; sin ningún efecto sobre PRESUPUESTO, RESERVA ni FECHA_BLOQUEADA. El gestor debe configurar las condiciones del espacio antes de reintentar.
...
flowchart TD
    ...
    M --> M2{GenerarPdfCondicionesPort.generar — condiciones configuradas?}
    M2 -->|null — sin config| M3[409 — CONDICIONES_NO_CONFIGURADAS sin efectos]
    M2 -->|ok| N[Tx SELECT…FOR UPDATE fila bloqueante]
    N --> O[INSERT PRESUPUESTO v1 enviado + congelado]
    O --> P[UPDATE RESERVA → pre_reserva + ttl_prereserva_dias + cond_part_enviadas_fecha=now + cond_part_firmadas=false]
    ...
    T --> U[Post-commit: PDF presupuesto + PDF condiciones → pdf_url UPDATE + E2 motor US-045]
```
...
8. Post-commit: se presenta al Gestor la factura de señal en borrador para revisión (disparo de UC-18 / US-022); el Gestor la aprobará y enviará vía E3 en UC-19 / US-023 (solo la factura de señal; las condicions particulars ya fueron enviadas con E2 al confirmar el presupuesto en UC-14)
...
11. La factura queda lista para que el Gestor ejecute el envío E3 mediante UC-19 / US-023 (`POST /reservas/{id}/facturas/senal/enviar`), que aprueba+envía en una sola operación atómica (solo la factura de señal; las condicions particulars se enviaron con E2 al confirmar el presupuesto en UC-14)
...
| **Actor Principal** | Gestor |
| **Actores Secundarios** | Sistema, Cliente |
| **Descripción** | El gestor gestiona el ciclo de vida del documento de condicions particulars: aprobación y envío manual al cliente (primer flujo, US-023) y registro de la copia firmada (segundo flujo, US-024). El primer envío de condicions particulars **ocurre en E2** (con el presupuesto, en `POST /reservas/{id}/presupuesto`), no en E3: desde el change `condiciones-idioma-e2-firma-banner`, `GenerarPresupuestoUseCase.confirmar()` adjunta el PDF de condiciones al presupuesto E2 y fija `RESERVA.cond_part_enviadas_fecha`. El Gestor pulsa "Enviar factura 40%" (E3) para aprobar y despachar la factura de señal **sin adjuntar condiciones** (E3 ya no lleva condicions particulars). El registro de la firma se realiza subiendo la copia firmada mediante `POST /reservas/{id}/condiciones-firmadas`; no transiciona estado. Implementado en **US-023** (changes `documentos-enviar-factura-senal-e3` 6.4b + `condiciones-particulares-e3-us023`) y **US-024** (change `firma-condiciones-particulares-us024`). Modificado en change `condiciones-idioma-e2-firma-banner`. |
| **Precondiciones** | Primer flujo (US-023): reserva en `reserva_confirmada`; factura de señal (`tipo='senal'`) en `estado='borrador'` con `pdf_url` no nulo; `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones ya enviadas con E2 al confirmar presupuesto); Gestor autenticado con rol gestor. Segundo flujo (US-024): `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas con E2); `RESERVA.estado ∈ {reserva_confirmada, evento_en_curso, post_evento}`; Gestor autenticado con rol gestor. |
| **Postcondiciones** | Primer flujo: `FACTURA(senal).estado = 'enviada'`; `COMUNICACION E3` con `estado='enviado'`; `AUDIT_LOG`. `RESERVA.cond_part_enviadas_fecha` y `cond_part_firmadas` **no se modifican en E3** (fueron fijados al confirmar el presupuesto en UC-14). Segundo flujo: nueva fila `DOCUMENTO(tipo='condiciones_particulares')` firmada; `RESERVA.cond_part_firmadas = true`; `RESERVA.cond_part_firmadas_fecha = now()`; `AUDIT_LOG accion='actualizar'`; `RESERVA.estado` sin cambio. |
| **Prioridad** | Alta |
| **Frecuencia** | Alta |
...
| **Entidades afectadas** | Primer flujo: FACTURA (UPDATE estado borrador→enviada, fecha_emision), COMUNICACION (INSERT E3 enviado), AUDIT_LOG. RESERVA no se modifica en E3 desde el change `condiciones-idioma-e2-firma-banner` (los campos `cond_part_*` se escriben en UC-14). Segundo flujo: DOCUMENTO (INSERT tipo=condiciones_particulares, copia firmada — no idempotente), RESERVA (UPDATE cond_part_firmadas, cond_part_firmadas_fecha), AUDIT_LOG. Sin migración BD en ambos flujos. |

**Flujo Básico — aprobación y envío de E3 (US-023):**
1. La factura de señal fue generada en borrador por US-022 como efecto post-commit de la confirmación de la reserva (UC-18). Las condicions particulars **ya fueron enviadas con E2** al confirmar el presupuesto (UC-14), por lo que `RESERVA.cond_part_enviadas_fecha` ya no es nulo en este punto
...
4. En una única unidad de trabajo atómica (`tx + RLS`): envía E3 por `EnviarEmailPort` directo con la factura de señal como único adjunto (las condicions particulars **no se adjuntan en E3** desde el change `condiciones-idioma-e2-firma-banner`). Si el proveedor de email falla → `EmisionEnvioFallidoError` (502), **rollback total** (nada se consolida)
5. Tras confirmar E3: transiciona la FACTURA `borrador → enviada`; crea COMUNICACION E3 `enviado`; registra AUDIT_LOG. Los campos `RESERVA.cond_part_enviadas_fecha` y `cond_part_firmadas` **no se modifican** (ya fueron fijados en UC-14)
6. El sistema responde 200 con la factura emitida y `condPartEnviadasFecha` (refleja el valor ya existente desde UC-14)
...
3. Reutiliza el PDF de la factura de señal ya emitida — NO regenera ni duplica ningún documento. Las condicions particulars **no se reenvían** en este flujo (se enviaron con E2 al confirmar el presupuesto)
4. En una única unidad de trabajo atómica: envía E3 por `EnviarEmailPort` directo; si falla → rollback
5. Tras confirmar: crea nueva COMUNICACION E3 `es_reenvio=true, estado='enviado'` (esquiva el índice UNIQUE parcial); registra AUDIT_LOG. NO transiciona factura ni reserva. `RESERVA.cond_part_enviadas_fecha` no se actualiza en el reenvío E3
...
3. El sistema valida (guardas de precondición, autoritativas, antes de mutar):
   - `RESERVA.cond_part_enviadas_fecha` no nulo (condiciones enviadas con E2 al confirmar presupuesto, UC-14). Si es nulo → 409 `CONDICIONES_NO_ENVIADAS` "Las condiciones particulares no han sido enviadas al cliente aún"; sin efectos
...
**Re-firma (no idempotente por diseño — N2):** si `cond_part_firmadas` ya es `true` (p. ej. subir una versión más legible), la operación no se rechaza. El sistema crea otra fila `DOCUMENTO` de `tipo = 'condiciones_particulares'` (nueva versión), actualiza `cond_part_firmadas_fecha` al nuevo timestamp, mantiene `cond_part_firmadas = true` y conserva todas las versiones anteriores de `DOCUMENTO` y de binarios. `AUDIT_LOG.datos_anteriores.cond_part_firmadas = true` en este caso. El DOCUMENTO no firmado lo genera `GenerarPresupuestoUseCase.confirmar()` al enviar E2 (UC-14, change `condiciones-idioma-e2-firma-banner`); la copia firmada es versionable (una por registro de firma de US-024).
...
- **FA-04** (condiciones no configuradas — se aplica en UC-14, no en E3): desde el change `condiciones-idioma-e2-firma-banner`, la guarda `CONDICIONES_NO_CONFIGURADAS` se aplica al confirmar el presupuesto (UC-14), no aquí. Si de alguna forma `cond_part_enviadas_fecha` fuese nulo al llegar a E3 (flujo anómalo), el contrato devuelve el campo `null`; E3 sigue adelante con solo la factura de señal adjunta.
...
- **FA-01** (condiciones no enviadas): `cond_part_enviadas_fecha` nulo → 409 `CONDICIONES_NO_ENVIADAS`; sin efectos en BD. La acción "Registrar condiciones firmadas" no está disponible hasta que se haya confirmado el presupuesto (E2, UC-14), que es cuando `cond_part_enviadas_fecha` se fija.
...
| E3 | Acción manual del gestor: "Enviar factura 40%" — `POST /reservas/{id}/facturas/senal/enviar` (US-023, 6.4b; condicions particulars movidas a E2 en change `condiciones-idioma-e2-firma-banner`) | Factura 40% (señal) | Activa |
...
>
> **Condicions particulars movidas de E3 a E2 (change `condiciones-idioma-e2-firma-banner`, 2026-07-21):** hasta este change, E3 adjuntaba el PDF de condicions particulars (si estaba configurado). Desde este change, E2 adjunta las condicions particulars al confirmar el presupuesto (`POST /reservas/{id}/presupuesto`, `GenerarPresupuestoUseCase.confirmar()`). Si el tenant no tiene condicions configuradas, la confirmación del presupuesto devuelve 409 `CONDICIONES_NO_CONFIGURADAS` antes de abrir la transacción. E3 envía únicamente la factura de señal. El parámetro `idioma: 'es' | 'ca'` es ahora requerido en `GenerarPdfCondicionesPort.generar()` y la clave de almacenamiento del PDF es `condiciones/{tenantId}-{idioma}.pdf`.
>
