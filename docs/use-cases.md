...
> **Nota de trazabilidad US-051 — cambiar una fecha ya bloqueada:** este UC-04 cubre exclusivamente la asignación de fecha a una RESERVA en `2.a` (sin fecha previa). Para el caso en que la RESERVA **ya tiene** una fecha bloqueada (`sub_estado ∈ {2b, 2c, 2v}`), US-051 introduce una operación atómica dedicada — `POST /reservas/{id}/cambiar-fecha` — que libera la fecha antigua y bloquea la nueva en una única transacción (con manejo de cola FIFO). Esta operación reutiliza `bloquearFecha()`/`liberarFecha()` sin modificar la máquina de estados ni este UC; no reemplaza el flujo `2.a → 2.b` descrito aquí. Ver `er-diagram.md §3.7` y `architecture.md §2.4` para la especificación técnica de la operación atómica.
