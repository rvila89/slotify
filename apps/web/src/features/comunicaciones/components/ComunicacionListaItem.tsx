...
import { etiquetaSubtipoEmail } from '../lib/subtipo-labels';
 ...
 * Muestra `codigoEmail`, la etiqueta del `subtipo` de E1 cuando lo hay (change
 * `historial-completo-comunicaciones`: varias filas E1 semánticamente distintas),
 * badge de `estado`, `asunto`, `destinatarioEmail`, `fechaCreacion` y (si aplica)
 * `fechaEnvio`. Las filas `enviado`/`fallido` son de SOLO LECTURA; las `borrador`
 * (flag `accionable`) muestran la acción Revisar y enviar. Se apila en móvil (`<sm`)
 * y reparte en fila en `sm:`/`lg:` sin overflow.
 */
type Props = {
...
export const ComunicacionListaItem = ({ item, onRevisar }: Props) => {
  const etiquetaSubtipo = etiquetaSubtipoEmail(item.subtipo);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex min-w-0 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {etiquetaSubtipo && (
              <span
                data-testid="comunicacion-subtipo"
                className="inline-flex items-center rounded-full bg-brand-primary/10 px-2 py-0.5 font-body text-xs font-medium text-text-primary"
              >
                {etiquetaSubtipo}
              </span>
            )}
