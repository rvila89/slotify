/**
 * Adaptador del puerto `EnviarConfirmacionVisitaPort` (US-008 / §D-6) — email E6
 * (confirmación de visita programada).
 *
 * Dispara E6 POST-COMMIT REUTILIZANDO el motor real de US-045
 * (`DespacharEmailService.despachar`): NO se reinventa el envío ni el registro en
 * `COMUNICACION`. El motor crea la fila `COMUNICACION` (`codigo_email='E6'`), envía
 * por el transporte (fake en test/CI) y la promueve a `enviado`/`fallido`, trazando
 * el resultado en `AUDIT_LOG`. La idempotencia ante doble disparo la garantiza el
 * UNIQUE parcial `(reserva_id, codigo_email)` del propio motor.
 *
 * Tolerancia a fallo: el motor centraliza el try/catch del proveedor y NO propaga la
 * excepción; si aun así fallara la carga de datos, el use-case envuelve la llamada en
 * un try/catch tolerante (post-commit, la transición ya está comprometida).
 */
import { Injectable } from '@nestjs/common';
import { PrismaService } from '../../shared/prisma/prisma.service';
import { DespacharEmailService } from '../../comunicaciones/application/despachar-email.service';
import type {
  EnviarConfirmacionVisitaParams,
  EnviarConfirmacionVisitaPort,
  EnviarConfirmacionVisitaResultado,
} from '../application/programar-visita.use-case';

@Injectable()
export class ConfirmacionVisitaEmailAdapter implements EnviarConfirmacionVisitaPort {
  constructor(
